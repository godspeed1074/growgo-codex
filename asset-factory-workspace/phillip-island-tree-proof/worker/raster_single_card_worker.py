...
s=bpy.context.scene;s.render.resolution_x=900;s.render.resolution_y=900;s.render.resolution_percentage=100;s.render.image_settings.file_format='PNG';s.render.image_settings.color_mode='RGBA';s.world.color=(.12,.12,.12);s.render.filepath=os.path.join(OUT,'GROWGO_NATIVE_ROUNDED_SINGLE_CLUSTER_BLENDER_CLEAN.png');bpy.ops.render.render(write_still=True)
g=os.path.join(OUT,'GROWGO_NATIVE_ROUNDED_SINGLE_CLUSTER_CLEAN.glb');bpy.ops.export_scene.gltf(filepath=g,export_format='GLB',export_materials='EXPORT');json.dump({'source':os.path.basename(ATLAS),'uvRegion':[0,0,1,1],'materialCount':1,'textureCount':1,'alphaMode':'BLEND_OR_DITHERED','doubleSided':True,'glb':os.path.basename(g),'blender':bpy.app.version_string},open(os.path.join(OUT,'SINGLE_CLUSTER_CLEAN_RECEIPT.json'),'w'),indent=2)
