...
args=sys.argv[sys.argv.index('--')+1:]; artifacts,art,out=args[:3]; os.makedirs(out,exist_ok=True); bpy.ops.wm.read_factory_settings(use_empty=True)
...
plane('AWNING','PRES_AWNING_PLUM.png',74,627,161,260,'GG-PRES-AWNING-PLUM-COMMERCIAL-001','AWNING',-.27)
plane('FASCIA','PRES_FASCIA_STRUCTURE.png',28,672,12,163,'GG-FAC-FASCIA-BAND-SIMPLE-001','FASCIA',-.28)
plane('SIGN','PRES_SIGN_GROW_GOODS.png',158,542,55,113,'GG-PRES-SIGN-GROW-GOODS-001','SIGN',-.29)
...
render('COMMERCIAL_SIMPLE_SWAPPED_FACADE_V2_FRONT.png')
for d,l in [(15,'15_LEFT'),(-15,'15_RIGHT')]:
 r=math.radians(d);cam.location=(12*math.sin(r),-12*math.cos(r),2.2);cam.rotation_euler=((target-cam.location).to_track_quat('-Z','Y')).to_euler();render('COMMERCIAL_SIMPLE_SWAPPED_FACADE_V2_'+l+'.png')
cam.location=(0,-12,2.2);cam.rotation_euler=((target-cam.location).to_track_quat('-Z','Y')).to_euler();s.render.engine='BLENDER_WORKBENCH';render('COMMERCIAL_SIMPLE_SWAPPED_FACADE_V2_COMPONENT_ID.png');[setattr(o,'show_wire',True) for o in objects if hasattr(o.data,'polygons')];render('COMMERCIAL_SIMPLE_SWAPPED_FACADE_V2_WIREFRAME.png')
json.dump({'status':'PASS','blenderVersion':bpy.app.version_string,'layout':'WINDOW_LEFT_DOOR_RIGHT','newStructuralGeometryTriangles':0,'parentFacadeRequired':'NO','masks':'NO','anonymousGeometry':0,'componentIds':[o.get('componentId') for o in objects]},open(os.path.join(out,'COMMERCIAL_SIMPLE_SWAPPED_FACADE_V2_RESULT.json'),'w'),indent=2)
bpy.ops.wm.save_as_mainfile(filepath=os.path.join(out,'GG-BLD-FACADE-SIMPLE-SHOP-MIRRORED-001_V2.blend'))
