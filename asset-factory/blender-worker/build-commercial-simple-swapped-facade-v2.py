...
args=sys.argv[sys.argv.index('--')+1:]; artifacts,art,out=args[:3]; flavor=args[3] if len(args)>3 else 'GROW_GOODS'; prefix='MIZTA_PIZZA_V4' if flavor=='MIZTA_PIZZA_V4' else ('MIZTA_PIZZA_V3' if flavor=='MIZTA_PIZZA_V3' else 'COMMERCIAL_SIMPLE_SWAPPED_FACADE_V2'); os.makedirs(out,exist_ok=True); bpy.ops.wm.read_factory_settings(use_empty=True)
...
def teal_backdrop():
 me=bpy.data.meshes.new('PIZZA_BACKDROP_MESH');me.from_pydata([(X(106),-.30,Z(496)),(X(350),-.30,Z(496)),(X(350),-.30,Z(307)),(X(106),-.30,Z(307))],[],[(0,1,2,3)]);o=bpy.data.objects.new('PIZZA_DISPLAY_BACKDROP',me);bpy.context.collection.objects.link(o);m=bpy.data.materials.new('PRES_PIZZA_DISPLAY_TEAL_BACKDROP');m.use_nodes=True;n=m.node_tree.nodes;n.clear();outn=n.new('ShaderNodeOutputMaterial');e=n.new('ShaderNodeEmission');e.inputs['Color'].default_value=(.015,.16,.20,1);m.node_tree.links.new(e.outputs[0],outn.inputs[0]);me.materials.append(m);o['assetId']='GG-PRES-WINDOW-DISPLAY-PIZZA-001';o['componentId']='PIZZA_DISPLAY_BACKDROP';o['parentFacadeDependency']='NONE';o['anonymousGeometry']=False;objects.append(o)
def glass_layer():
 me=bpy.data.meshes.new('WINDOW_GLASS_ONLY_MESH');me.from_pydata([(X(106),-.33,Z(496)),(X(350),-.33,Z(496)),(X(350),-.33,Z(307)),(X(106),-.33,Z(307))],[],[(0,1,2,3)]);o=bpy.data.objects.new('WINDOW_GLASS_ONLY',me);bpy.context.collection.objects.link(o);m=bpy.data.materials.new('GG_MAT_WINDOW_GLASS_TEAL_UNLIT');m.use_nodes=True;n=m.node_tree.nodes;n.clear();outn=n.new('ShaderNodeOutputMaterial');e=n.new('ShaderNodeEmission');e.inputs['Color'].default_value=(.05,.42,.48,1);tr=n.new('ShaderNodeBsdfTransparent');mix=n.new('ShaderNodeMixShader');mix.inputs[0].default_value=.12;m.node_tree.links.new(tr.outputs[0],mix.inputs[1]);m.node_tree.links.new(e.outputs[0],mix.inputs[2]);m.node_tree.links.new(mix.outputs[0],outn.inputs[0]);m.surface_render_method='DITHERED';me.materials.append(m);o['assetId']='GG-BLD-WINDOW-SHOP-LARGE-002';o['componentId']='WINDOW_GLASS_ONLY';o['parentFacadeDependency']='NONE';o['anonymousGeometry']=False;objects.append(o)
def shell_box(name,asset,loc,dims):
 bpy.ops.mesh.primitive_cube_add(size=1,location=loc);o=bpy.context.object;o.name=name;o.dimensions=dims;bpy.ops.object.transform_apply(location=False,rotation=False,scale=True);o['assetId']=asset;o['componentId']=name;o['layer']='LAYER_A_MODULE';o['runtimeArtifact']='APPROVED_SHELL_LAYER_A_INSTANCE';o['parentFacadeDependency']='NONE';o['anonymousGeometry']=False;objects.append(o)
...
awning_file='MIZTA_AWNING.png' if flavor in ('MIZTA_PIZZA_V3','MIZTA_PIZZA_V4') else 'PRES_AWNING_PLUM.png'; sign_file='MIZTA_SIGN.png' if flavor in ('MIZTA_PIZZA_V3','MIZTA_PIZZA_V4') else 'PRES_SIGN_GROW_GOODS.png'
awning_asset='GG-PRES-AWNING-MIZTA-STRIPED-001' if flavor in ('MIZTA_PIZZA_V3','MIZTA_PIZZA_V4') else 'GG-PRES-AWNING-PLUM-COMMERCIAL-001'; sign_asset='GG-PRES-SIGN-MIZTA-PIZZA-001' if flavor in ('MIZTA_PIZZA_V3','MIZTA_PIZZA_V4') else 'GG-PRES-SIGN-GROW-GOODS-001'
plane('AWNING',awning_file,74,627,161,260,awning_asset,'AWNING_STRIPED_PRESENTATION' if flavor in ('MIZTA_PIZZA_V3','MIZTA_PIZZA_V4') else 'AWNING',-.27)
plane('FASCIA','PRES_FASCIA_STRUCTURE.png',28,672,12,163,'GG-FAC-FASCIA-BAND-SIMPLE-001','FASCIA',-.28)
plane('SIGN',sign_file,158,542,55,113,sign_asset,'SIGN_MIZTA_PIZZA' if flavor in ('MIZTA_PIZZA_V3','MIZTA_PIZZA_V4') else 'SIGN',-.29)
if flavor in ('MIZTA_PIZZA_V3','MIZTA_PIZZA_V4'):
 if flavor!='MIZTA_PIZZA_V4': teal_backdrop()
 plane('PIZZA_DISPLAY','MIZTA_PIZZA_V4_CLEAN.png' if flavor=='MIZTA_PIZZA_V4' else 'MIZTA_PIZZA.png',142,314,325,482,'GG-PRES-WINDOW-DISPLAY-PIZZA-001','PIZZA_DISPLAY',-.31)
 if flavor=='MIZTA_PIZZA_V4':
  glass_layer()
  # Registered Simple Shop shell instances: intentionally simple hidden geometry for QA.
  shell_box('SHELL_LEFT','GG-BLD-WALL-SIDE-COMMERCIAL-SIMPLE-001',(-2.35,1.25,1.85),(.10,2.5,3.7));shell_box('SHELL_RIGHT','GG-BLD-WALL-SIDE-COMMERCIAL-SIMPLE-001',(2.35,1.25,1.85),(.10,2.5,3.7));shell_box('SHELL_REAR','GG-BLD-WALL-REAR-COMMERCIAL-SIMPLE-001',(0,2.5,1.85),(4.8,.10,3.7));shell_box('SHELL_FLOOR','GG-BLD-FLOOR-FOOTPRINT-SIMPLE-001',(0,1.25,.08),(4.8,2.5,.12));shell_box('SHELL_ROOF_BACK','GG-BLD-ROOF-BACK-SHELL-COMMERCIAL-001',(0,1.25,3.75),(4.8,2.5,.12));shell_box('SHELL_CORNER_L','GG-BLD-CORNER-COMMERCIAL-SIMPLE-001',(-2.35,2.5,1.85),(.12,.12,3.7));shell_box('SHELL_CORNER_R','GG-BLD-CORNER-COMMERCIAL-SIMPLE-001',(2.35,2.5,1.85),(.12,.12,3.7))
...
json.dump({'status':'PASS','executionMode':'REAL_BLENDER_WORKER_EXECUTION','blenderVersion':bpy.app.version_string,'assetId':'GG-BLD-SHOP-SIMPLE-MIZTA-PIZZA-001@1.0.0' if flavor in ('MIZTA_PIZZA_V3','MIZTA_PIZZA_V4') else 'GG-BLD-FACADE-SIMPLE-SHOP-MIRRORED-001@1.0.0','layout':'WINDOW_LEFT_DOOR_RIGHT','newStructuralGeometryTriangles':0,'presentationTriangles':10 if flavor=='MIZTA_PIZZA_V4' else (8 if flavor=='MIZTA_PIZZA_V3' else 0),'parentFacadeRequired':'NO','masks':'NO','anonymousGeometry':0,'componentIds':[o.get('componentId') for o in objects]},open(os.path.join(out,prefix+'_RESULT.json'),'w'),indent=2)
bpy.ops.wm.save_as_mainfile(filepath=os.path.join(out,('GG-BLD-SHOP-SIMPLE-MIZTA-PIZZA-001_'+('V4' if flavor=='MIZTA_PIZZA_V4' else 'V3')+'_CANDIDATE.blend' if flavor in ('MIZTA_PIZZA_V3','MIZTA_PIZZA_V4') else 'GG-BLD-FACADE-SIMPLE-SHOP-MIRRORED-001_V2.blend')))
