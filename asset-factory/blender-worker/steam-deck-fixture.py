...
render_path=os.path.join(artifact_dir,'BLENDER_RENDER.png'); blend_path=os.path.join(artifact_dir,'STEAM_DECK_FIXTURE_OUTPUT.blend'); scene.render.filepath=render_path
...
json.dump(stats,open(os.path.join(artifact_dir,'BLENDER_STATS.json'),'w'),sort_keys=True)
...
json.dump({'certificateVersion':'GG-STEAM-DECK-RUNTIME-CERTIFICATE-1.0.0','status':'PASS','workerRuntimeId':'STEAM-DECK-BLENDER-FLATPAK','blenderVersion':bpy.app.version_string,'pythonExecuted':True,'controlledRender':True,'cleanExit':True},open(os.path.join(out_dir,'WORKER_BLENDER_RUNTIME_CERTIFICATE.json'),'w'),sort_keys=True)
artifacts=[]
for logical_id, filename, artifact_type in [('BLENDER_WORKER_RESULT','BLENDER_WORKER_RESULT.json','JSON'),('WORKER_RUNTIME_CERTIFICATE','WORKER_BLENDER_RUNTIME_CERTIFICATE.json','JSON'),('BLENDER_RENDER','BLENDER_RENDER.png','PNG_RGBA'),('VERSIONED_BLEND','STEAM_DECK_FIXTURE_OUTPUT.blend','BLEND'),('BLENDER_STATS','BLENDER_STATS.json','JSON')]:
    source = os.path.join(out_dir, filename) if logical_id in ('BLENDER_WORKER_RESULT','WORKER_RUNTIME_CERTIFICATE') else os.path.join(artifact_dir, filename)
