...
export const buildingCoastalCottageRuntimePreviewBindingStates = Object.freeze([
  "requested",
  "fetching",
  "parsing",
  "loaded",
...
export const buildingCoastalCottageRuntimePreviewBindingDefinition = deepFreeze({
  glbRuntimeLoadId: "ATLAS_RUNTIME_GLB_LOAD_BUILDING_COASTAL_COTTAGE_001",
  ...
  loaderState: {
    ...
    allowedStates: buildingCoastalCottageRuntimePreviewBindingStates,
  ...
  renderResult: {
    ...
    displayMode: "runtime-glb-cottage-preview",
    fallbackPlaceholder: "HOUSE_PLACEHOLDER",
...
const supportedStates = new Set(buildingCoastalCottageRuntimePreviewBindingStates);
...
export async function loadBuildingCoastalCottageRuntimePreviewBinding(
  ...
  try {
    ...
    if (error?.name !== "BuildingCoastalCottageRuntimePreviewBindingValidationError") {
      throw error;
    }
    return Object.freeze({
      ok: false,
...
export function createBuildingCoastalCottageRuntimePreviewBinding(
  rawDefinition = buildingCoastalCottageRuntimePreviewBindingDefinition
) {
...
function validateCompatibility(definition, importBridge) {
  if (definition.assetId !== importBridge.glbRegistration.assetId) {
    throw createValidationError(
      ...
      "Building coastal cottage runtime preview binding assetId must match the GLB import bridge assetId."
    );
  }
  if (
    ...
    throw createValidationError(
      ...
      "Building coastal cottage runtime preview binding must target the gameplay GLB output."
    );
  }
  if (
    importBridge.runtimePreviewBinding.renderPayload.rendererProfile !==
      definition.renderResult.rendererProfile ||
    importBridge.runtimePreviewBinding.lodSelector.gameplay !==
      definition.glbReference.lodKey
  ) {
    throw createValidationError(
      "runtime_preview_binding_mismatch",
      "Building coastal cottage runtime preview binding must preserve the passive renderer profile and gameplay LOD selection."
    );
  }
}
...
function validateLoadedRuntime(definition) {
  if (!definition.validationResult.binaryParsed) {
    throw createValidationError(
      ...
      "Building coastal cottage runtime preview binding must mark binaryParsed true after successful parsing."
    );
  }
  if (!definition.validationResult.meshExtracted) {
    throw createValidationError(
      "mesh_extraction_invalid",
      "Building coastal cottage runtime preview binding must extract at least one mesh primitive."
    );
  }
  if (!definition.validationResult.materialExtracted) {
    throw createValidationError(
      "material_extraction_invalid",
      "Building coastal cottage runtime preview binding must extract at least one material."
    );
  }
}
...
function parseBinaryGlb(arrayBuffer) {
  if (!(arrayBuffer instanceof ArrayBuffer) || arrayBuffer.byteLength < 20) {
    throw createValidationError(
      "invalid_glb_binary",
      "Building coastal cottage runtime preview binding requires a valid GLB ArrayBuffer."
    );
  }
  ...
  if (
    ...
    throw createValidationError(
      "invalid_glb_binary",
      "Building coastal cottage runtime preview binding encountered an invalid GLB header."
    );
  }
  ...
  if (view.getUint32(16, true) != 0x4E4F534A) {
    throw createValidationError(
      "invalid_glb_binary",
      "Building coastal cottage runtime preview binding requires a leading JSON chunk."
    );
  }
  ...
  try {
    ...
    throw createValidationError(
      "invalid_glb_binary",
      "Building coastal cottage runtime preview binding could not parse the embedded glTF JSON."
    );
  }
...
function normalizeRuntimeBindingDefinition(rawDefinition) {
  const definition = asPlainObject(
    rawDefinition,
    "buildingCoastalCottageRuntimePreviewBinding"
  );
  for (const fieldName of buildingCoastalCottageRuntimePreviewBindingRequiredFields) {
    if (!(fieldName in definition)) {
      throw createValidationError(
        "missing_required_field",
        `Building coastal cottage runtime preview binding definition is missing ${fieldName}.`
      );
    }
  }
  if (
    normalizeString(definition.assetId, "assetId") !==
    "BUILDING_COASTAL_COTTAGE_001"
  ) {
    throw createValidationError(
      "asset_identity_invalid",
      "Building coastal cottage runtime preview binding is limited to BUILDING_COASTAL_COTTAGE_001."
    );
  }
  return deepFreeze({
...
function normalizeGlbReference(rawValue) {
  ...
  if (!supportedLodKeys.has(lodKey)) {
    throw createValidationError(
      "invalid_lod_key",
      "Building coastal cottage runtime preview binding lodKey must be supported."
    );
  }
  return deepFreeze({
...
function normalizeLoaderState(rawValue) {
  ...
  const currentState = normalizeString(
    loaderState.currentState,
    "loaderState.currentState"
  );
  if (!supportedStates.has(currentState)) {
    throw createValidationError(
      "invalid_loader_state",
      "Building coastal cottage runtime preview binding currentState must be supported."
    );
  }
  return deepFreeze({
    currentState,
      ...
      (Array.isArray(loaderState.allowedStates) ? loaderState.allowedStates : []).map(
        (state, index) => {
          const value = normalizeString(
            state,
            `loaderState.allowedStates[${index}]`
          );
          if (!supportedStates.has(value)) {
            throw createValidationError(
              "invalid_loader_state",
              "Building coastal cottage runtime preview binding allowedStates must be supported."
            );
          }
          return value;
        }
...
function normalizeMeshResult(rawValue) {
  ...
  const vertices = Array.isArray(meshResult.projectedVertices)
    ? meshResult.projectedVertices.map((vertex, index) =>
        normalizeVertex(vertex, index)
      )
    : [];
  if (vertices.length < 3) {
    throw createValidationError(
      "invalid_mesh_result",
      "Building coastal cottage runtime preview binding meshResult.projectedVertices must describe a drawable polygon."
    );
  }
  return deepFreeze({
    ...
    vertexCount: normalizeNonNegativeInteger(
      meshResult.vertexCount,
      "meshResult.vertexCount"
    ),
...
function normalizeValidationResult(rawValue) {
  ...
  return deepFreeze({
    ...
    rendererCompatibilityValid: Boolean(
...
function createValidationError(code, message) {
  return Object.assign(new Error(message), {
    code,
    name: "BuildingCoastalCottageRuntimePreviewBindingValidationError"
  });
}
