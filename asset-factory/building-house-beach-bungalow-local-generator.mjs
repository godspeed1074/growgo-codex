...
export const buildingHouseBeachBungalowLocalGeneratorRequiredFields =
  Object.freeze([
    "assetId",
    "recipeId",
    "familyId",
...
export const buildingHouseBeachBungalowLocalGeneratorDefinition = deepFreeze({
  assetId: "BUILDING_HOUSE_BEACH_BUNGALOW_001",
  recipeId: "RECIPE_HOUSE_BEACH_BUNGALOW_001",
  familyId: "FAMILY_HOUSE_COASTAL",
  scriptLocation:
    "asset-factory/local-blender-scripts/generate_building_house_beach_bungalow.py",
  expectedOutputLocation:
    "asset-factory-workspace/production/HOUSE_BEACH_BUNGALOW_FAMILY_001/export",
  executionInstructions: {
    blenderCommand:
      "blender --factory-startup --python asset-factory/local-blender-scripts/generate_building_house_beach_bungalow.py -- --output-dir asset-factory-workspace/production/HOUSE_BEACH_BUNGALOW_FAMILY_001/export --auto-quit",
    commandArguments: [
      "--factory-startup",
      "--python",
      "asset-factory/local-blender-scripts/generate_building_house_beach_bungalow.py",
  ...
  validationMetadata: {
    ...
    coastalExpansionResolutionValidated: true,
...
export function createBuildingHouseBeachBungalowLocalGenerator(
  rawDefinition = buildingHouseBeachBungalowLocalGeneratorDefinition
) {
  return normalizeDefinition(rawDefinition);
}

export function buildBuildingHouseBeachBungalowLocalBlenderCommand(
  rawDefinition = buildingHouseBeachBungalowLocalGeneratorDefinition
) {
  return normalizeDefinition(rawDefinition).executionInstructions.blenderCommand;
}

export function validateBuildingHouseBeachBungalowLocalGenerator(
  rawDefinition = buildingHouseBeachBungalowLocalGeneratorDefinition
) {
  try {
    ...
    if (definition.scriptLocation !== expectedScriptPath) {
      throw createValidationError(
        ...
        "Building house beach bungalow local generator script location must match the approved assembly-test script."
      );
    }
    ...
    if (!definition.executionInstructions.blenderCommand.startsWith("blender ")) {
      throw createValidationError(
        "command_mismatch",
        "Building house beach bungalow local generator command must begin with the Blender executable."
      );
    }
    ...
    if (
      ...
      throw createValidationError(
        "command_mismatch",
        "Building house beach bungalow local generator command must include the approved script path and output directory."
      );
    }
    ...
    if (!definition.executionInstructions.blenderCommand.includes("--auto-quit")) {
      throw createValidationError(
        "command_mismatch",
        "Building house beach bungalow local generator command must request automatic quit for controlled GUI execution."
      );
    }
    ...
    if (!definition.executionInstructions.localExecutionOnly) {
      throw createValidationError(
        ...
        "Building house beach bungalow local generator must remain local-execution-only."
      );
    }
    ...
    if (definition.executionInstructions.preferredExecutionMode !== "gui") {
      throw createValidationError(
        ...
        "Building house beach bungalow local generator must prefer GUI execution."
      );
    }
    ...
    if (error?.name !== "BuildingHouseBeachBungalowLocalGeneratorValidationError") {
      throw error;
    }
...
function normalizeDefinition(rawDefinition) {
  const definition = asPlainObject(
    rawDefinition,
    "building house beach bungalow local generator"
  );
...
function normalizeValidationMetadata(rawValidationMetadata) {
  ...
  return deepFreeze({
    ...
    coastalExpansionResolutionValidated: normalizeBoolean(
      validationMetadata.coastalExpansionResolutionValidated,
      "validationMetadata.coastalExpansionResolutionValidated"
    ),
...
function assertRequiredFields(definition) {
  for (const fieldName of buildingHouseBeachBungalowLocalGeneratorRequiredFields) {
    if (!Object.prototype.hasOwnProperty.call(definition, fieldName)) {
      throw createValidationError(
        "missing_required_field",
        `Building house beach bungalow local generator is missing required field ${fieldName}.`
      );
    }
  }
}
...
function createValidationError(code, message) {
  ...
  error.name = "BuildingHouseBeachBungalowLocalGeneratorValidationError";
  error.code = code;
  return error;
}
