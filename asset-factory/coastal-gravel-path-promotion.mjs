...
const REGISTRATION_FILENAME = "coastal-gravel-path-registration.json";
const CATALOG_FILENAME = "coastal-gravel-path-development-catalog-entry.json";
const VERSION_FILENAME = "coastal-gravel-path-v001-version-record.json";
const PROMOTION_FILENAME = "coastal-gravel-path-v001-promotion.json";
const VISUAL_APPROVAL_FILENAME = "coastal-gravel-path-visual-approval.json";
const EXPORT_VALIDATION_FILENAME = "coastal-gravel-path-export-validation.json";
const EXPORT_MANIFEST_FILENAME = "coastal-gravel-path-export-manifest.json";
const SOURCE_VERIFICATION_FILENAME = "coastal-gravel-path-source-verification.json";
...
export function buildCoastalGravelPathV001Promotion(options = {}) {
  ...
  const promotionRecord = deepFreeze({
    schemaId: "COASTAL_GRAVEL_PATH_V001_CATALOG_PROMOTION_001",
...
export function writeCoastalGravelPathV001Promotion(options = {}) {
  const promotion = buildCoastalGravelPathV001Promotion(options);
  fs.writeFileSync(
...
function assertPromotionPreconditions({
  ...
  const checks = [
    [registration.assetId === "COASTAL_GRAVEL_PATH_001", "registration identity mismatch"],
    [catalog.assetId === "COASTAL_GRAVEL_PATH_001", "catalog identity mismatch"],
    [versionRecord.assetId === "COASTAL_GRAVEL_PATH_001", "version identity mismatch"],
    [visualApproval.assetId === "COASTAL_GRAVEL_PATH_001", "visual approval identity mismatch"],
    [exportValidation.assetId === "COASTAL_GRAVEL_PATH_001", "export validation identity mismatch"],
    [exportManifest.assetId === "COASTAL_GRAVEL_PATH_001", "export manifest identity mismatch"],
    [sourceVerification.assetId === "COASTAL_GRAVEL_PATH_001", "source verification identity mismatch"],
    [registration.recipeId === "COASTAL_GRAVEL_PATH_RECIPE_001", "registration recipe mismatch"],
    [catalog.recipeId === "COASTAL_GRAVEL_PATH_RECIPE_001", "catalog recipe mismatch"],
    [exportManifest.recipeId === "COASTAL_GRAVEL_PATH_RECIPE_001", "export manifest recipe mismatch"],
    [
  ...
  if (failed) {
    throw new Error(`Gravel-path promotion blocked: ${failed[1]}.`);
  }
}

function readJson(filename) {
  if (!fs.existsSync(filename)) {
    throw new Error(`Gravel-path promotion blocked: missing ${filename}.`);
  }
