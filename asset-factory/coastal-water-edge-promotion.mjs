...
const VALIDATION_LOCATION =
  "asset-factory-workspace/production/COASTAL_NATURE_FAMILY_001/validation";
const SOURCE_LOCATION =
  "asset-factory-workspace/production/COASTAL_NATURE_FAMILY_001/source";

const REGISTRATION_FILENAME = "coastal-water-edge-registration.json";
const CATALOG_FILENAME = "coastal-water-edge-development-catalog-entry.json";
const VERSION_FILENAME = "coastal-water-edge-v001-version-record.json";
const PROMOTION_FILENAME = "coastal-water-edge-v001-promotion.json";
const VISUAL_APPROVAL_FILENAME = "coastal-water-edge-visual-approval.json";
const EXPORT_VALIDATION_FILENAME = "coastal-water-edge-export-validation.json";
const EXPORT_MANIFEST_FILENAME = "coastal-water-edge-export-manifest.json";
const SOURCE_VERIFICATION_FILENAME = "coastal-water-edge-source-verification.json";
...
export function buildCoastalWaterEdgeV001Promotion(options = {}) {
  ...
  const promotionRecord = deepFreeze({
    schemaId: "COASTAL_WATER_EDGE_V001_CATALOG_PROMOTION_001",
...
export function writeCoastalWaterEdgeV001Promotion(options = {}) {
  const promotion = buildCoastalWaterEdgeV001Promotion(options);
  fs.writeFileSync(
...
function assertPromotionPreconditions({
  ...
  const checks = [
    [registration.assetId === "COASTAL_WATER_EDGE_001", "registration identity mismatch"],
    [catalog.assetId === "COASTAL_WATER_EDGE_001", "catalog identity mismatch"],
    [versionRecord.assetId === "COASTAL_WATER_EDGE_001", "version identity mismatch"],
    [visualApproval.assetId === "COASTAL_WATER_EDGE_001", "visual approval identity mismatch"],
    [exportValidation.assetId === "COASTAL_WATER_EDGE_001", "export validation identity mismatch"],
    [exportManifest.assetId === "COASTAL_WATER_EDGE_001", "export manifest identity mismatch"],
    [sourceVerification.assetId === "COASTAL_WATER_EDGE_001", "source verification identity mismatch"],
    [registration.recipeId === "COASTAL_WATER_EDGE_RECIPE_001", "registration recipe mismatch"],
    [catalog.recipeId === "COASTAL_WATER_EDGE_RECIPE_001", "catalog recipe mismatch"],
    [exportManifest.recipeId === "COASTAL_WATER_EDGE_RECIPE_001", "export manifest recipe mismatch"],
    [
  ...
  if (failed) {
    throw new Error(`Water-edge promotion blocked: ${failed[1]}.`);
  }
}

function readJson(filename) {
  if (!fs.existsSync(filename)) {
    throw new Error(`Water-edge promotion blocked: missing ${filename}.`);
  }
