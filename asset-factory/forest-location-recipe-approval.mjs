...
const APPROVAL_FILENAME = "forest-location-recipe-001-approval.json";
const CATALOG_FILENAME = "forest-location-recipe-001-approved-catalog-entry.json";
const VERSION_FILENAME = "forest-location-recipe-001-v001-version-record.json";
const REPORT_FILENAME = "forest-location-recipe-001-approval-report.md";
...
function loadInputs(cwd) {
  ...
  if (regenerationValidation.status !== "pass") {
    throw new Error(
      "Forest location recipe approval blocked: regeneration validation must pass first."
    );
  }
  if (previewReviewValidation.status !== "pass") {
    throw new Error(
      "Forest location recipe approval blocked: regenerated preview review validation must pass first."
    );
  }
  if (previewComparison.overallStatus !== "APPROVED_FOR_RECIPE_APPROVAL") {
    throw new Error(
      "Forest location recipe approval blocked: regenerated preview comparison is not approval-ready."
    );
  }
...
function buildApprovalRecord(inputs, options = {}) {
  ...
  const checks = [
    { name: "improvements_confirmed", ok: true },
    {
      ...
      ok:
        ...
        inputs.regeneratedDependencyMap.deferredAssets.some(
          (asset) => asset.assetId === "TREE_EUCALYPTUS_001"
        )
    },
    {
    ...
    {
      name: "unsupported_assets_excluded",
  ...
  return deepFreeze({
    schemaId: "FOREST_LOCATION_RECIPE_001_APPROVAL_RECORD_001",
    ...
    dependencyIds,
    deferredDependencyIds: inputs.regeneratedDependencyMap.deferredAssets.map(
      (asset) => asset.assetId
    ),
    ...
    comparisonSummary: {
      ...
      canopyCorridorQuality:
        inputs.previewComparison.review.canopyCorridorQuality.status,
      clearingQuality: inputs.previewComparison.review.clearingQuality.status,
      explorationInterest:
...
function buildApprovedCatalogEntry(inputs, approvalRecord) {
  return deepFreeze({
    schemaId: "FOREST_LOCATION_RECIPE_001_APPROVED_CATALOG_ENTRY_001",
    ...
    placementSummary: {
      ...
      deferredDependencies: inputs.regeneratedDependencyMap.deferredAssets.map(
        (asset) => asset.assetId
      )
    },
    performanceSummary: inputs.regeneratedPlan.mobilePerformanceLimits,
...
function buildVersionRecord(inputs, approvalRecord) {
  const versionHash = hashHex(
    ...
    JSON.stringify(approvalRecord.deferredDependencyIds)
  );

  return deepFreeze({
    schemaId: "FOREST_LOCATION_RECIPE_001_VERSION_RECORD_001",
    recipeId: approvalRecord.recipeId,
...
function buildReport(approvalRecord, catalogEntry, versionRecord) {
  return `# FOREST_LOCATION_RECIPE_001 Approval Report

Status: Refined forest location recipe approved
...
export function buildForestLocationRecipeApprovalPackage(options = {}) {
...
export function writeForestLocationRecipeApprovalPackage(options = {}) {
  const cwd = options.cwd ?? process.cwd();
  const inputs = loadInputs(cwd);
  const built = buildForestLocationRecipeApprovalPackage(options);
  ...
  writeJson(built.paths.approvalPath, built.approvalRecord);
  writeJson(built.paths.catalogPath, built.catalogEntry);
  writeJson(built.paths.versionPath, built.versionRecord);
  fs.writeFileSync(built.paths.reportPath, `${built.report}\n`);
...
if (isDirectRun) {
  writeForestLocationRecipeApprovalPackage();
}
