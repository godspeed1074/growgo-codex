"""Build TREE_EUCALYPTUS_001_v006 as an art-directed review candidate.
...
ATLAS_NAME = f"{STEM}_FOLIAGE_ATLAS_512.png"
...
LOD = {
    "CLOSE": {"sides": 10, "path_step": 2, "cards": 10, "secondaries": 4},
    "GAMEPLAY": {"sides": 8, "path_step": 2, "cards": 10, "secondaries": 4},
    "MAP": {"sides": 5, "path_step": 1, "cards": 5, "secondaries": 2},
}
...
def flat_material(name, colour):
    m = bpy.data.materials.new(f"MAT_EUCALYPTUS_{name}_006")
...
def foliage_material(atlas_path):
    m = bpy.data.materials.new("MAT_EUCALYPTUS_FOLIAGE_ATLAS_MASK_006")
    m.use_nodes = True
...
def tube(name, points, r0, r1, sides, subdivisions, mat, parent, collection):
...
def bark_patches(lod, mats, parent, collection):
    if lod == "MAP":
        return
    # Large designed planes, intentionally sparse and graphic.
    specs = [
        ((-.27, -.58, 1.18), (.62, 1.22), .04, "PEACH"),
        ((.40, -.43, 2.38), (.50, 1.05), -.14, "TAN"),
        ((-.36, -.36, 3.55), (.54, .92), .10, "RUST"),
        ((.34, -.30, 4.56), (.46, .84), -.09, "PEACH"),
        ((-.25, -.24, 5.55), (.40, .74), .08, "TAN"),
        ((.22, -.18, 6.52), (.34, .62), -.06, "PEACH"),
        ((-.12, -.13, 7.45), (.30, .52), .05, "RUST"),
    ]
    verts, faces, mat_ids = [], [], []
    keys = ["TAN", "PEACH", "RUST"]
    for (x, y, z), (w, h), skew, key in specs:
        base = len(verts)
        verts += [(x-w*.48, y, z-h*.48), (x+w*.50, y, z-h*.36), (x+w*(.36+skew), y, z+h*.50), (x-w*(.42-skew), y, z+h*.35)]
        faces.append((base, base+1, base+2, base+3))
        mat_ids.append(keys.index(key))
    mesh = bpy.data.meshes.new(f"{ASSET_ID}_{lod}_GRAPHIC_BARK_MESH")
    mesh.from_pydata(verts, [], faces)
    for key in keys:
        mesh.materials.append(mats[key])
    for polygon, index in zip(mesh.polygons, mat_ids):
        polygon.material_index = index
    obj = bpy.data.objects.new(f"{ASSET_ID}_{lod}_GRAPHIC_BARK", mesh)
    collection.objects.link(obj)
    obj.parent = parent
...
def append_card(verts, faces, uvs, centre, width, height, yaw, pitch, tile):
    # A lightly bowed six-vertex card avoids a mechanically flat silhouette.
    local = [
        (-width/2, 0, -height/2), (0, -.055*width, -height/2), (width/2, 0, -height/2),
        (-width/2, 0, height/2), (0, -.055*width, height/2), (width/2, 0, height/2),
    ]
    ...
    faces.extend([(base, base+1, base+4), (base, base+4, base+3), (base+1, base+2, base+5), (base+1, base+5, base+4)])
    (u0, v0), (u1, v1) = UV_TILES[tile]
    uv_by_vertex = [(u0,v0),((u0+u1)/2,v0),(u1,v0),(u0,v1),((u0+u1)/2,v1),(u1,v1)]
    for face in [(0,1,4),(0,4,3),(1,2,5),(1,5,4)]:
...
def build_lod(lod, mats, foliage_mat):
    ...
    for name, r0, r1, path in MAJOR_PATHS:
        tube(f"{ASSET_ID}_{lod}_{name}", path, r0, r1, LOD[lod]["sides"], LOD[lod]["path_step"], mats["CREAM"], root, collection)
    for name, r0, r1, path in SECONDARY_PATHS[:LOD[lod]["secondaries"]]:
        tube(f"{ASSET_ID}_{lod}_{name}", path, r0, r1, max(5, LOD[lod]["sides"]-2), LOD[lod]["path_step"], mats["CREAM"], root, collection)
    roots(lod, mats, root, collection)
    bark_patches(lod, mats, root, collection)
...
def records(output_dir, preview_dir, atlas_path):
    metadata = {
        ...
        "construction": {
            ...
            "textureAtlas": {"filename": atlas_path.name, "dimensions": [512, 512], "alphaMode": "MASK", "bytes": atlas_path.stat().st_size},
            "materials": 5,
        },
        "protectedHistoricalVersions": ["v001", "v002", "v003", "v004", "v005"],
    ...
    for label, payload in (("metadata", metadata), ("authoring-manifest", authoring), ("manifest", manifest)):
        (output_dir / f"tree-eucalyptus-v006-{label}.json").write_text(json.dumps(payload, indent=2) + "\n")
