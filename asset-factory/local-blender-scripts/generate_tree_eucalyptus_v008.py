"""Build TREE_EUCALYPTUS_001_v008 as an art-directed review candidate.
...
MAJOR_PATHS = [
    ("MAIN_TRUNK", 0.84, 0.30, [(0, 0, 0), (-.18, .04, 1.45), (.02, .12, 2.9), (-.20, .12, 4.35), (.08, .02, 5.85), (-.08, -.04, 7.35), (.10, -.10, 8.85), (-.04, -.22, 10.45)]),
    ("LOWER_LEFT_SWEEP", .52, .15, [(-.05, .02, 4.8), (-.66, .20, 5.55), (-1.45, .54, 6.35), (-2.30, 1.02, 7.05), (-3.12, 1.34, 7.62)]),
    ("MID_LEFT_ARCH", .47, .13, [(-.12, .05, 6.15), (-.82, -.15, 7.0), (-1.65, -.48, 7.95), (-2.48, -.75, 8.85), (-3.22, -.78, 9.42)]),
    ("UPPER_LEFT_FORK", .43, .11, [(-.04, .00, 7.45), (-.60, .31, 8.45), (-1.22, .44, 9.55), (-1.78, .42, 10.65), (-2.42, .42, 11.55)]),
    ("CENTRAL_CROWN_FORK", .42, .11, [(.06, -.06, 8.1), (.14, -.17, 9.25), (.08, -.25, 10.55), (-.05, -.30, 12.15)]),
    ("MID_RIGHT_SWEEP", .50, .14, [(.08, -.02, 5.35), (.74, -.25, 6.15), (1.52, -.60, 7.05), (2.30, -.90, 8.05), (3.16, -1.02, 9.02)]),
    ("UPPER_RIGHT_ARCH", .44, .12, [(.14, .08, 6.85), (.78, .43, 7.9), (1.36, .66, 9.05), (1.82, .68, 10.15), (2.34, .62, 11.28)]),
    ("LOWER_RIGHT_REACH", .38, .10, [(1.30, -.54, 7.0), (1.95, .02, 7.02), (2.55, .66, 7.30), (3.20, 1.22, 7.62)]),
]
...
# Eight unequal islands aggressively match the reference crown rhythm.
# centre xyz, half-width, half-depth, half-height, dominant tile, density.
ISLANDS = [
    ("DOMINANT_UPPER_CROWN", (-.35, -.24, 12.70), 2.05, 1.42, 1.28, 0, 1.00),
    ("UPPER_LEFT_CROWN", (-2.62, .42, 11.45), 1.76, 1.25, 1.10, 0, .92),
    ("UPPER_RIGHT_CROWN", (2.36, .70, 11.30), 1.72, 1.28, 1.08, 0, .92),
    ("BROAD_MIDDLE_LEFT", (-3.28, -.72, 9.42), 1.82, 1.32, 1.10, 2, .95),
    ("BROAD_MIDDLE_RIGHT", (3.18, -1.00, 9.08), 1.80, 1.34, 1.08, 2, .95),
    ("LOWER_LEFT_GROUP", (-3.08, 1.30, 7.63), 1.60, 1.18, .98, 0, .82),
    ("CENTRAL_INTERIOR", (-.55, -2.62, 9.42), 1.24, 1.12, .82, 3, .65),
    ("LOWER_RIGHT_GROUP", (3.08, 1.42, 7.68), 1.55, 1.18, .95, 2, .80),
]

LOD = {
    "CLOSE": {"sides": 12, "path_step": 3, "cards": 40, "secondaries": 4},
    "GAMEPLAY": {"sides": 10, "path_step": 3, "cards": 34, "secondaries": 4},
...
def flat_material(name, colour):
    m = bpy.data.materials.new(f"MAT_EUCALYPTUS_{name}_008")
...
def foliage_material(atlas_path):
    m = bpy.data.materials.new("MAT_EUCALYPTUS_FOLIAGE_ATLAS_MASK_008")
    m.use_nodes = True
...
def tube(name, points, r0, r1, sides, subdivisions, mat, parent, collection, surface_mats=None):
    ...
    if surface_mats:
        ...
        if "MAIN_TRUNK" in name:
            ...
            regions = [(.06, .27, .54, 2, 1), (.30, .50, .16, 2, 2),
                       (.56, .74, .70, 2, 1), (.78, .90, .38, 1, 3)]
        else:
...
def soften_junctions(lod, mats, parent, collection):
    if lod == "MAP":
        return
    segments = 12 if lod == "CLOSE" else 10
    rings = 6
    swells = [
        ((-.04, .02, 5.35), (.76, .66, 1.05)),
        ((-.04, .03, 6.65), (.66, .58, .92)),
        ((.02, .00, 7.70), (.58, .52, .82)),
        ((.05, -.06, 8.45), (.50, .46, .70)),
    ]
...
def roots(lod, mats, parent, collection):
    ...
    tube(f"{ASSET_ID}_{lod}_SCULPTED_ROOT_COLLAR", [(0, 0, 0), (-.06, .02, 1.10)],
         1.12, .66, max(8, LOD[lod]["sides"]), 3 if lod != "MAP" else 1, mats["CREAM"], parent, collection)
    specs = [
        ("FRONT_LEFT", [(0, 0, .82), (-.48, -.48, .36), (-1.24, -.78, .20)], .48),
        ("FRONT_RIGHT", [(0, 0, .78), (.45, -.54, .34), (1.10, -.94, .20)], .45),
        ("LONG_LEFT", [(-.05, .02, .72), (-.78, .22, .34), (-1.55, .38, .20)], .43),
        ("RIGHT_SIDE", [(.06, .03, .70), (.66, .40, .32), (1.32, .72, .20)], .40),
        ("REAR_SHORT", [(-.02, .08, .64), (-.28, .62, .30), (-.52, 1.06, .20)], .35),
        ("LEFT_REAR", [(-.04, .06, .62), (-.58, .52, .30), (-1.02, .90, .20)], .34),
    ]
    count = 6 if lod != "MAP" else 4
    for name, path, width in specs[:count]:
        tube(f"{ASSET_ID}_{lod}_ROOT_{name}", path, width, .14, max(6, LOD[lod]["sides"] - 2), 2, mats["CREAM"], parent, collection)
        if lod != "MAP":
            end = path[-1]
            ellipsoid(f"{ASSET_ID}_{lod}_ROOT_TOE_{name}", (end[0], end[1], .22),
                      (width*.72, width*.58, .14), 8, 4, mats["CREAM"], parent, collection)
    ...
    for island_index, (name, centre, half_w, half_d, half_h, dominant, density) in enumerate(ISLANDS):
        ...
        for card_index, (ox, oy, oz, yaw, pitch, scale) in enumerate(CARD_LAYOUT[:island_cards]):
            if card_index < 8:
                ...
                tile = dominant if card_index % 3 else 2  # mid-green body
            elif card_index < 34:
                tile = 1 if card_index % 4 else 0  # light upper/front crown
            else:
                tile = 2 if card_index % 2 else 0  # drooping edge foliage
...
def records(output_dir, preview_dir, atlas_path):
    metadata = {
        ...
        "previousCandidateVersion": "v007",
        "targetRevisionVersion": VERSION,
        "status": "PENDING_OPERATOR_REVIEW",
        "authoredDimensionsMetres": {"height": 14.0, "crownWidth": 9.0, "crownDepth": 7.0},
        "construction": {
            "method": "aggressive reference-shape match with eight unequal canopy umbrellas, explicit colour-depth layers, softened fork swells, rounded root toes, and integrated bark face regions",
        ...
        "protectedHistoricalVersions": ["v001", "v002", "v003", "v004", "v005", "v006", "v007"],
    ...
    for label, payload in (("metadata", metadata), ("authoring-manifest", authoring), ("manifest", manifest)):
        (output_dir / f"tree-eucalyptus-v008-{label}.json").write_text(json.dumps(payload, indent=2) + "\n")
