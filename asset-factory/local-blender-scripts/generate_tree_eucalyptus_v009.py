"""Build TREE_EUCALYPTUS_001_v009 as an art-directed review candidate.
...
LOD = {
    "CLOSE": {"sides": 12, "path_step": 3, "cards": 44, "secondaries": 4},
    "GAMEPLAY": {"sides": 10, "path_step": 3, "cards": 36, "secondaries": 4},
...
def flat_material(name, colour):
    m = bpy.data.materials.new(f"MAT_EUCALYPTUS_{name}_009")
...
def foliage_material(atlas_path):
    m = bpy.data.materials.new("MAT_EUCALYPTUS_FOLIAGE_ATLAS_MASK_009")
    m.use_nodes = True
...
def roots(lod, mats, parent, collection):
    ...
    tube(f"{ASSET_ID}_{lod}_SCULPTED_ROOT_COLLAR", [(0, 0, 0), (-.10, .02, 1.18)],
         1.28, .70, max(10, LOD[lod]["sides"]), 4 if lod != "MAP" else 1, mats["CREAM"], parent, collection)
    if lod != "MAP":
        ellipsoid(f"{ASSET_ID}_{lod}_BROAD_PLANTED_ROOT_MOUND", (-.04, -.02, .38),
                  (1.28, 1.02, .58), 12 if lod == "CLOSE" else 10, 6, mats["CREAM"], parent, collection)
    specs = [
...
def records(output_dir, preview_dir, atlas_path):
    metadata = {
        ...
        "construction": {
            "method": "final trunk-sculpture refinement with smoother fork blending, broader planted roots, face-bound bark streaks, and controlled inner-canopy fullness",
    ...
    for label, payload in (("metadata", metadata), ("authoring-manifest", authoring), ("manifest", manifest)):
        (output_dir / f"tree-eucalyptus-v009-{label}.json").write_text(json.dumps(payload, indent=2) + "\n")
