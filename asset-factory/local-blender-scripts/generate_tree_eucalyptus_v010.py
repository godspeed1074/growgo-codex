"""Build TREE_EUCALYPTUS_001_v010 as an art-directed review candidate.
...
ISLANDS = [
    ("DOMINANT_FULL_CROWN", (-.20, -.18, 12.35), 2.42, 1.72, 1.55, 0, 1.00),
    ("UPPER_LEFT_CROWN", (-2.48, .30, 11.35), 1.92, 1.48, 1.28, 0, .98),
    ("UPPER_RIGHT_CROWN", (2.30, .55, 11.25), 1.88, 1.48, 1.26, 0, .98),
    ("BROAD_MIDDLE_LEFT", (-3.08, -.62, 9.48), 1.96, 1.50, 1.28, 2, 1.00),
    ("BROAD_MIDDLE_RIGHT", (3.02, -.86, 9.18), 1.94, 1.52, 1.26, 2, 1.00),
    ("LOWER_LEFT_GROUP", (-2.86, 1.12, 7.72), 1.78, 1.38, 1.12, 0, .92),
    ("CENTRAL_INTERIOR", (-.42, -2.12, 9.70), 1.48, 1.34, .98, 3, .84),
    ("LOWER_RIGHT_GROUP", (2.92, 1.22, 7.78), 1.74, 1.38, 1.10, 2, .90),
]
...
def flat_material(name, colour):
    m = bpy.data.materials.new(f"MAT_EUCALYPTUS_{name}_010")
...
def foliage_material(atlas_path):
    m = bpy.data.materials.new("MAT_EUCALYPTUS_FOLIAGE_ATLAS_MASK_010")
    m.use_nodes = True
...
def records(output_dir, preview_dir, atlas_path):
    metadata = {
        ...
        "construction": {
            "method": "full-canopy cartoony pass with art-directed branch skeleton, broader planted roots, face-bound graphic bark, authored foliage islands, and reusable alpha-masked eucalyptus foliage atlas cards",
    ...
    for label, payload in (("metadata", metadata), ("authoring-manifest", authoring), ("manifest", manifest)):
        (output_dir / f"tree-eucalyptus-v010-{label}.json").write_text(json.dumps(payload, indent=2) + "\n")
