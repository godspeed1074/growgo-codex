"""Build TREE_EUCALYPTUS_001_v011 as an art-directed review candidate.
...
ISLANDS = [
    ("FULL_ROUND_CROWN", (0.0, -.10, 10.65), 2.70, 2.10, 2.15, 0, 1.00),
    ("CROWN_LEFT", (-1.72, .18, 10.35), 1.72, 1.70, 1.70, 2, .98),
    ("CROWN_RIGHT", (1.72, .24, 10.30), 1.72, 1.70, 1.68, 2, .98),
    ("LOW_LEFT_ROUND", (-2.25, -.38, 9.05), 1.55, 1.55, 1.45, 0, .96),
    ("LOW_RIGHT_ROUND", (2.25, -.35, 9.00), 1.55, 1.55, 1.45, 0, .96),
    ("LOW_FRONT", (0.0, -1.45, 8.95), 1.72, 1.28, 1.30, 3, .94),
    ("UPPER_LEFT_HIGHLIGHT", (-.92, -.48, 11.95), 1.25, 1.20, 1.15, 0, .86),
    ("UPPER_RIGHT_HIGHLIGHT", (.98, -.38, 11.88), 1.25, 1.20, 1.15, 0, .86),
]
...
def flat_material(name, colour):
    m = bpy.data.materials.new(f"MAT_EUCALYPTUS_{name}_011")
...
def foliage_material(atlas_path):
    m = bpy.data.materials.new("MAT_EUCALYPTUS_FOLIAGE_ATLAS_MASK_011")
    m.use_nodes = True
...
def records(output_dir, preview_dir, atlas_path):
    ...
    for label, payload in (("metadata", metadata), ("authoring-manifest", authoring), ("manifest", manifest)):
        (output_dir / f"tree-eucalyptus-v011-{label}.json").write_text(json.dumps(payload, indent=2) + "\n")
