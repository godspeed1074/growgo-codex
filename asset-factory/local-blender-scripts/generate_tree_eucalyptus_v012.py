"""Build TREE_EUCALYPTUS_001_v012 as an art-directed review candidate.
...
MAJOR_PATHS = [
    ("MAIN_TRUNK", .58, .22, [(0, 0, 0), (-.10, .03, .9), (.05, .06, 1.8), (-.10, .05, 2.7), (.05, 0, 3.65), (0, -.08, 4.6)]),
    ("LOWER_LEFT_SWEEP", .34, .10, [(-.03, .02, 2.6), (-.45, .10, 3.15), (-.92, .20, 3.72), (-1.45, .30, 4.25)]),
    ("MID_LEFT_ARCH", .30, .08, [(-.08, .02, 3.2), (-.55, -.12, 3.75), (-1.10, -.22, 4.25), (-1.55, -.25, 4.65)]),
    ("UPPER_LEFT_FORK", .27, .07, [(-.02, .0, 3.8), (-.42, .16, 4.35), (-.82, .20, 4.85), (-1.18, .18, 5.20)]),
    ("CENTRAL_CROWN_FORK", .26, .07, [(.02, -.04, 3.9), (.02, -.08, 4.55), (-.04, -.12, 5.05)]),
    ("MID_RIGHT_SWEEP", .32, .09, [(.06, -.02, 2.95), (.48, -.12, 3.45), (.98, -.18, 3.95), (1.48, -.20, 4.35)]),
    ("UPPER_RIGHT_ARCH", .28, .07, [(.08, .05, 3.65), (.48, .18, 4.2), (.90, .22, 4.75), (1.25, .18, 5.10)]),
    ("LOWER_RIGHT_REACH", .24, .06, [(1.0, -.15, 3.35), (1.45, .05, 3.55), (1.72, .22, 3.8)]),
]

SECONDARY_PATHS = [
    ("LEFT_TOP_SUPPORT", .12, .035, [(-.60, .20, 4.75), (-1.02, .05, 5.10), (-1.30, -.10, 5.35)]),
    ("RIGHT_TOP_SUPPORT", .12, .035, [(.60, .22, 4.70), (1.02, .08, 5.05), (1.35, -.04, 5.28)]),
    ("LEFT_LOWER_SUPPORT", .10, .03, [(-.72, .25, 3.55), (-1.15, .12, 3.85), (-1.45, .0, 4.05)]),
    ("CENTRE_BACK_SUPPORT", .10, .03, [(.0, -.20, 4.15), (-.25, -.65, 4.55), (-.35, -.95, 4.82)]),
]
...
ISLANDS = [
    ("FULL_ROUND_CROWN", (0.0, -.05, 5.45), 1.65, 1.35, 1.45, 0, 1.00),
    ("CROWN_LEFT", (-1.02, .10, 5.35), 1.05, 1.12, 1.15, 2, .98),
    ("CROWN_RIGHT", (1.02, .12, 5.33), 1.05, 1.12, 1.14, 2, .98),
    ("LOW_LEFT_ROUND", (-1.28, -.28, 4.55), .95, .95, .92, 0, .96),
    ("LOW_RIGHT_ROUND", (1.28, -.25, 4.52), .95, .95, .92, 0, .96),
    ("LOW_FRONT", (0.0, -1.00, 4.50), 1.12, .90, .88, 3, .94),
    ("UPPER_LEFT_HIGHLIGHT", (-.58, -.35, 6.15), .72, .78, .72, 0, .86),
    ("UPPER_RIGHT_HIGHLIGHT", (.60, -.30, 6.12), .72, .78, .72, 0, .86),
]
...
def flat_material(name, colour):
    m = bpy.data.materials.new(f"MAT_EUCALYPTUS_{name}_012")
...
def foliage_material(atlas_path):
    m = bpy.data.materials.new("MAT_EUCALYPTUS_FOLIAGE_ATLAS_MASK_012")
    m.use_nodes = True
...
def roots(lod, mats, parent, collection):
    ...
    tube(f"{ASSET_ID}_{lod}_SCULPTED_ROOT_COLLAR", [(0, 0, 0), (-.10, .02, .65)],
         .78, .46, max(10, LOD[lod]["sides"]), 4 if lod != "MAP" else 1, mats["CREAM"], parent, collection)
    if lod != "MAP":
        ellipsoid(f"{ASSET_ID}_{lod}_BROAD_PLANTED_ROOT_MOUND", (-.04, -.02, .38),
                  (.86, .70, .38), 12 if lod == "CLOSE" else 10, 6, mats["CREAM"], parent, collection)
    specs = [
...
def records(output_dir, preview_dir, atlas_path):
    ...
    for label, payload in (("metadata", metadata), ("authoring-manifest", authoring), ("manifest", manifest)):
        (output_dir / f"tree-eucalyptus-v012-{label}.json").write_text(json.dumps(payload, indent=2) + "\n")
