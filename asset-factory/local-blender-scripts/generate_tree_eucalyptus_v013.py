"""Build TREE_EUCALYPTUS_001_v013 as an art-directed review candidate.
...
BARK = {
    "CREAM": (0.34, 0.18, 0.08, 1.0),
    "TAN": (0.24, 0.11, 0.045, 1.0),
    "PEACH": (0.42, 0.22, 0.09, 1.0),
    "RUST": (0.16, 0.07, 0.025, 1.0),
}
...
MAJOR_PATHS = [("MAIN_TRUNK", .52, .28, [(0, 0, 0), (-.04, 0, .8), (.04, .02, 1.65), (-.02, 0, 2.5), (0, 0, 3.35)])]
...
ISLANDS = [
    ("FULL_ROUND_CROWN", (0.0, 0, 4.65), 1.65, 1.35, 1.35, 0, 1.00),
...
def flat_material(name, colour):
    m = bpy.data.materials.new(f"MAT_EUCALYPTUS_{name}_013")
...
def foliage_material(atlas_path):
    m = bpy.data.materials.new("MAT_EUCALYPTUS_FOLIAGE_ATLAS_MASK_013")
    m.use_nodes = True
...
def records(output_dir, preview_dir, atlas_path):
    metadata = {
        ...
        "construction": {
            "method": "direct reference recreation: simple brown trunk, minimal roots, and one compact scalloped green canopy; no eucalyptus-specific branching or bark treatment",
    ...
    for label, payload in (("metadata", metadata), ("authoring-manifest", authoring), ("manifest", manifest)):
        (output_dir / f"tree-eucalyptus-v013-{label}.json").write_text(json.dumps(payload, indent=2) + "\n")
