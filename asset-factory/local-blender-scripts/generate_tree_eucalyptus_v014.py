"""Build TREE_EUCALYPTUS_001_v014 as an art-directed review candidate.
...
MAJOR_PATHS = [("MAIN_TRUNK", .58, .24, [(0, 0, 0), (-.16, .02, .75), (.16, .00, 1.5), (-.18, .02, 2.25), (.14, 0, 3.0), (-.08, .02, 3.75), (.04, 0, 4.5)])]
...
def flat_material(name, colour):
    m = bpy.data.materials.new(f"MAT_EUCALYPTUS_{name}_014")
...
def foliage_material(atlas_path):
    m = bpy.data.materials.new("MAT_EUCALYPTUS_FOLIAGE_ATLAS_MASK_014")
    m.use_nodes = True
...
def records(output_dir, preview_dir, atlas_path):
    ...
    for label, payload in (("metadata", metadata), ("authoring-manifest", authoring), ("manifest", manifest)):
        (output_dir / f"tree-eucalyptus-v014-{label}.json").write_text(json.dumps(payload, indent=2) + "\n")
