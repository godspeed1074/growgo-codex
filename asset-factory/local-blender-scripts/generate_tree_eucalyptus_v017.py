"""Build TREE_EUCALYPTUS_001_v017 as an art-directed review candidate.
...
def flat_material(name, colour):
    m = bpy.data.materials.new(f"MAT_EUCALYPTUS_{name}_017")
...
def foliage_material(atlas_path):
    m = bpy.data.materials.new("MAT_EUCALYPTUS_FOLIAGE_ATLAS_MASK_017")
    m.use_nodes = True
...
def records(output_dir, preview_dir, atlas_path):
    ...
    for label, payload in (("metadata", metadata), ("authoring-manifest", authoring), ("manifest", manifest)):
        (output_dir / f"tree-eucalyptus-v017-{label}.json").write_text(json.dumps(payload, indent=2) + "\n")
