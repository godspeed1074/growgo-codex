"""
Manual Blender-internal export resume script for TREE_BOTTLEBRUSH_001.

This script is intended to be run by a human from Blender 4.2 LTS after
opening the final saved bottlebrush blend file. It does not regenerate the
tree. It exports the three approved LOD roots one at a time, validates each
temporary GLB, and finalizes each export only after validation passes.
"""
...
EXPORT_START_MARKER = "S187_TREE_BOTTLEBRUSH_EXPORT_START"
EXPORT_COMPLETE_MARKER = "S187_TREE_BOTTLEBRUSH_EXPORT_COMPLETE"
EXPORT_SKIP_PREFIX = "S187_TREE_BOTTLEBRUSH_EXPORT_SKIP:"
EXPORT_FAILURE_PREFIX = "S187_TREE_BOTTLEBRUSH_EXPORT_FAILURE:"
ASSET_GUARD_MARKER = "S187_TREE_BOTTLEBRUSH_ASSET_GUARD_OK"
RECIPE_GUARD_MARKER = "S187_TREE_BOTTLEBRUSH_RECIPE_GUARD_OK"
VERSION_GUARD_MARKER = "S187_TREE_BOTTLEBRUSH_VERSION_GUARD_OK"

EXPORT_SEQUENCE = (
    ("close", "LOD_CLOSE", f"{VERSIONED_ASSET_STEM}_LOD_CLOSE.glb", f"{ASSET_ID}_LOD_CLOSE_ROOT"),
    ("gameplay", "LOD_GAMEPLAY", f"{VERSIONED_ASSET_STEM}_LOD_GAMEPLAY.glb", f"{ASSET_ID}_LOD_GAMEPLAY_ROOT"),
    ("map", "LOD_MAP", f"{VERSIONED_ASSET_STEM}_LOD_MAP.glb", f"{ASSET_ID}_LOD_MAP_ROOT"),
)
...
def ensure_blender_version():
    ...
    if version[:2] != (4, 2):
        fail(
            f"Expected Blender 4.2.x for bottlebrush export resume, found {version!r}.",
...
def ensure_output_directory():
    ...
    print(f"Resolved bottlebrush export directory: {output_dir}")
    sys.stdout.flush()
...
def find_open_blend_path():
    ...
    if filepath is None or not str(filepath).strip():
        fail(
            "The bottlebrush blend must be opened before running the resume script.",
...
def ensure_asset_identity():
    ...
    if asset_collection is None or any(root is None for root in lod_roots):
        fail(
            "The open blend does not contain the expected bottlebrush root collection and LOD roots.",
    ...
    if asset_collection.get("growgo_identity_contract_v2") != expected_contract_json:
        fail(
            "The open blend does not contain the expected Tree Bottlebrush v2 identity contract metadata.",
...
def collect_id_properties(target):
    keys = (
        ...
        "growgo_exported_identity_source",
        "growgo_identity_anchor",
    )
    metadata = {}
    for key in keys:
...
def validate_pre_export_identity(root_object, lod_label):
    ...
    if not any(
        ...
        or metadata_matches_dependency_contract(metadata, None)
...
def validate_export(filepath, expected_metrics):
    ...
    if not metrics["assetIdentityPreserved"]:
        raise RuntimeError(f"{filepath.name} did not preserve the bottlebrush asset identity.")
    if not metrics["metadataIdentityPreserved"]:
        raise RuntimeError(f"{filepath.name} did not preserve bottlebrush metadata identity.")
    if not metrics["anchorIdentityPreserved"]:
        raise RuntimeError(f"{filepath.name} did not preserve the bottlebrush identity anchor.")
...
def export_single_lod(lod_key, lod_label, final_filename, root_name):
    ...
    emit_marker(f"S187_TREE_BOTTLEBRUSH_EXPORT_LOD_START:{lod_label}")
    export_root(root_object, temp_path)
    verified_metrics = validate_export(temp_path, expected_metrics)
    temp_path.replace(final_path)
    emit_marker(f"S187_TREE_BOTTLEBRUSH_EXPORT_LOD_COMPLETE:{lod_label}")
