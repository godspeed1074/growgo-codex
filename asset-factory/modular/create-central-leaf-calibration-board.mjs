...
const oldCrop = crop(fullOld), newCrop = crop(fullNew), targetAlpha = new Uint8Array(targetMask.w * targetMask.h); for (let i = 0; i < targetAlpha.length; i++) targetAlpha[i] = targetMask.rgba[i * 4 + 3] ? 1 : 0;
const renderedGeometryMask = idAlphaCrop(idRender), renderedBeautyMask = greenMask(newCrop), measured = iou(targetAlpha, renderedGeometryMask), beautyMeasured = iou(targetAlpha, renderedBeautyMask), targetStats = maskStats(targetAlpha), renderStats = maskStats(renderedGeometryMask);
...
const report = { status: numericPass ? 'BLOCKED_CENTRAL_LEAF_DIRECT_VISION' : 'BLOCKED_CENTRAL_LEAF_NUMERIC_GATE', leafId: 'LEAF_001', numeric: { targetContourIoU: Number((contourSpec.targetSpecificContourScore ?? contourSpec.targetPartitionHullScore ?? 0).toFixed(4)), renderedGeometryMaskIoU: Number(measured.value.toFixed(4)), beautyColourMaskIoU: Number(beautyMeasured.value.toFixed(4)), silhouetteIoU: Number(measured.value.toFixed(4)), centreErrorPx: Number(centreErrorPx.toFixed(3)), widthErrorPercent: Number(widthErrorPercent.toFixed(3)), heightErrorPercent: Number(heightErrorPercent.toFixed(3)), tipErrorPx: Number(tipErrorPx.toFixed(3)), angleErrorDeg: Number(angleErrorDeg.toFixed(3)), targetMaskStats: targetStats, renderedMaskStats: renderStats }, directVision: { sameOverallLeaf: 'NO', tipShape: 'NEEDS_CORRECTION', leftShoulder: 'NEEDS_CORRECTION', rightShoulder: 'NEEDS_CORRECTION', baseTaper: 'NEEDS_CORRECTION', asymmetry: 'NEEDS_CORRECTION', centralRidge: 'PASS', lightDarkFacets: 'PASS', papercutCharacter: 'NEEDS_CORRECTION', genericLeafAppearance: 'YES' }, shadingMode: 'C', board: 'CENTRAL_LEAF_EXACT_RECONSTRUCTION_BOARD.png', renders: ['CENTRAL_LEAF_MODE_A_FLAT_FACETED.png', 'CENTRAL_LEAF_MODE_B_CONTROLLED_NORMALS.png', 'CENTRAL_LEAF_MODE_C_PAPERCUT_FACETS.png', 'CENTRAL_LEAF_FINAL_FRONT.png', 'CENTRAL_LEAF_FINAL_3Q.png', 'CENTRAL_LEAF_FINAL_SIDE.png', 'CENTRAL_LEAF_WIREFRAME_FRONT.png'], otherLeavesChanged: false, flowersChanged: false, planterChanged: false };
