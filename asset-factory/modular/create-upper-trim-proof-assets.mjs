...
for(const [side,d] of Object.entries(defs)){
 ...
 for(let y=0;y<h;y++)for(let x=0;x<w;x++){const sx=x0+x,sy=y0+y,s=(sy*im.w+sx)*4,p=(y*w+x)*4;crop[p]=im.rgba[s];crop[p+1]=im.rgba[s+1];crop[p+2]=im.rgba[s+2];crop[p+3]=255;const rr=im.rgba[s],gg=im.rgba[s+1],bb=im.rgba[s+2];const brownContext=rr>gg*1.25&&gg>bb*1.15;const inside=sx>=d.art[0]&&sx<d.art[2]&&sy>=d.art[1]&&sy<d.art[3]&&rr>120&&gg>90&&gg>bb*1.08&&rr>gg*1.08&&!brownContext;if(inside){mask[p]=mask[p+1]=mask[p+2]=255;mask[p+3]=255;}}
