...
import { roadCoastalPrototypeAssetPackageDefinition } from "./road-coastal-prototype-asset-package.mjs";

export const roadCoastalLocalGeneratorRequiredFields = Object.freeze([
  "assetId",
  "scriptLocation",
...
export const roadCoastalLocalGeneratorDefinition = deepFreeze({
  assetId: "ROAD_COASTAL_001",
  scriptLocation:
    "asset-factory/local-blender-scripts/generate_road_coastal.py",
  expectedOutputLocation:
    "asset-factory-workspace/production/COASTAL_INFRASTRUCTURE_FAMILY_001/export",
  executionInstructions: {
    blenderCommand:
      "blender --background --python asset-factory/local-blender-scripts/generate_road_coastal.py -- --output-dir asset-factory-workspace/production/COASTAL_INFRASTRUCTURE_FAMILY_001/export",
    commandArguments: [
      "--background",
      "--python",
      "asset-factory/local-blender-scripts/generate_road_coastal.py",
...
export function createRoadCoastalLocalGenerator(
  rawDefinition = roadCoastalLocalGeneratorDefinition
) {
  return normalizeGeneratorDefinition(rawDefinition);
}

export function buildRoadCoastalLocalBlenderCommand(
  rawDefinition = roadCoastalLocalGeneratorDefinition
) {
...
export function validateRoadCoastalLocalGenerator(
  rawDefinition = roadCoastalLocalGeneratorDefinition,
  options = {}
) {
  try {
    ...
    if (error?.name !== "RoadCoastalLocalGeneratorValidationError") {
      throw error;
    }
...
function validateCompatibility(definition, assetPackage) {
  if (definition.assetId !== assetPackage.assetId) {
    throw createValidationError(
      "asset_id_mismatch",
      "Road coastal local generator assetId must match the prototype asset package."
    );
  }
  ...
  if (definition.expectedOutputLocation !== assetPackage.exportMetadata.assetPackageLocation) {
    throw createValidationError(
      ...
      "Road coastal local generator output location must match the prototype asset package export location."
    );
  }
  ...
  if (definition.scriptLocation !== expectedScriptPath) {
    throw createValidationError(
      ...
      "Road coastal local generator script location must match the approved Python script."
    );
  }
  ...
  if (!definition.executionInstructions.blenderCommand.startsWith("blender ")) {
    throw createValidationError(
      "command_mismatch",
      "Road coastal local generator command must start with the Blender executable."
    );
  }
  ...
  if (
    ...
    throw createValidationError(
      "command_mismatch",
      "Road coastal local generator command must include the script location and output directory."
    );
  }
  ...
  if (!definition.executionInstructions.localExecutionOnly) {
    throw createValidationError(
      ...
      "Road coastal local generator must remain local-execution-only."
    );
  }
}

function normalizeGeneratorDefinition(rawDefinition) {
  const definition = asPlainObject(rawDefinition, "road coastal local generator");
...
function assertRequiredFields(definition) {
  for (const fieldName of roadCoastalLocalGeneratorRequiredFields) {
    if (!Object.prototype.hasOwnProperty.call(definition, fieldName)) {
      throw createValidationError(
        "missing_required_field",
        `Road coastal local generator is missing required field ${fieldName}.`
      );
    }
  }
}
...
function createValidationError(code, message) {
  return Object.assign(new Error(message), {
    code,
    name: "RoadCoastalLocalGeneratorValidationError"
  });
}
