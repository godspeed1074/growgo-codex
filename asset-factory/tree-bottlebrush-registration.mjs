...
export const treeBottlebrushRegistrationFilename =
  "tree-bottlebrush-registration.json";
export const treeBottlebrushDevelopmentCatalogFilename =
  "tree-bottlebrush-development-catalog-entry.json";
export const treeBottlebrushRegistrationSchemaId =
  "TREE_BOTTLEBRUSH_REGISTRATION_RECORD_001";
export const treeBottlebrushDevelopmentCatalogSchemaId =
  "TREE_BOTTLEBRUSH_DEVELOPMENT_CATALOG_ENTRY_001";
...
export function buildTreeBottlebrushRegistrationRecord(
  ...
  if (!verification.registrationGate.ready) {
    throw createRegistrationError(
      ...
      `Cannot register ${definition.assetId} until final bottlebrush outputs validate.`
    );
  }
  ...
  const manifest = readJsonRequired(
    path.join(outputDirectory, "tree-bottlebrush-manifest.json"),
    "tree bottlebrush manifest"
  );
  ...
  const validation = readJsonRequired(
    path.join(outputDirectory, "tree-bottlebrush-validation.json"),
    "tree bottlebrush validation"
  );
  ...
  const verifiedMap = findVerifiedOutput(
    verification,
  ...
  return deepFreeze({
    schemaId: treeBottlebrushRegistrationSchemaId,
    ...
    verification: deepFreeze({
      ...
      recipeIdentityPreserved: [
        ...
        verifiedGameplay,
        verifiedMap
      ].every((entry) => entry.recipeIdentityPreserved === true),
      dependencyValidationPassed: [
        verifiedClose,
        verifiedGameplay,
        verifiedMap
      ].every((entry) => entry.dependencyIdentityPreserved === true),
      metadataIdentityPreserved: [
        verifiedClose,
        verifiedGameplay,
        verifiedMap
      ].every((entry) => entry.metadataIdentityPreserved === true),
...
export function writeTreeBottlebrushRegistrationRecord(
  rawDefinition = treeBottlebrushProductionRunDefinition,
  options = {}
) {
  const definition = normalizeDefinition(rawDefinition);
  const cwd = options.cwd ?? process.cwd();
  const outputDirectory = path.resolve(cwd, definition.outputLocation);
  const record = buildTreeBottlebrushRegistrationRecord(rawDefinition, options);
  const registrationPath = path.join(outputDirectory, treeBottlebrushRegistrationFilename);
...
export function buildTreeBottlebrushDevelopmentCatalogEntry(
  rawDefinition = treeBottlebrushProductionRunDefinition,
  options = {}
) {
  const definition = normalizeDefinition(rawDefinition);
  ...
  const registration =
    ...
    buildTreeBottlebrushRegistrationRecord(rawDefinition, options);
  const metadata = readJsonRequired(
    path.join(outputDirectory, "tree-bottlebrush-metadata.json"),
    "tree bottlebrush metadata"
  );
  ...
  return deepFreeze({
    schemaId: treeBottlebrushDevelopmentCatalogSchemaId,
...
export function writeTreeBottlebrushDevelopmentCatalogEntry(
  rawDefinition = treeBottlebrushProductionRunDefinition,
  options = {}
) {
  const definition = normalizeDefinition(rawDefinition);
  ...
  const registrationResult = options.registrationRecord
    ...
    : writeTreeBottlebrushRegistrationRecord(rawDefinition, options);
  const record = buildTreeBottlebrushDevelopmentCatalogEntry(rawDefinition, {
    ...options,
    registrationRecord: registrationResult.record
  });
  const catalogPath = path.join(outputDirectory, treeBottlebrushDevelopmentCatalogFilename);
...
export function writeTreeBottlebrushRegistrationAndDevelopmentCatalog(
  rawDefinition = treeBottlebrushProductionRunDefinition,
  options = {}
) {
  const registration = writeTreeBottlebrushRegistrationRecord(rawDefinition, options);
  const catalog = writeTreeBottlebrushDevelopmentCatalogEntry(rawDefinition, {
    ...options,
...
function findVerifiedOutput(verification, filename) {
  ...
  if (!entry || entry.classification !== "VERIFIED_COMPLETE") {
    throw createRegistrationError(
      ...
      `Expected verified output ${filename} was not available for bottlebrush registration.`
    );
  }
  return entry;
}
...
function createRegistrationError(code, message) {
  ...
  error.name = "TreeBottlebrushRegistrationError";
  error.code = code;
  return error;
}
