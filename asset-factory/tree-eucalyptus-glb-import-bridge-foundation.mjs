...
import {
  treeEucalyptusLocalGeneratorDefinition,
  validateTreeEucalyptusLocalGenerator
} from "./tree-eucalyptus-local-generator.mjs";

export const treeEucalyptusGlbImportBridgeFoundationRequiredFields = Object.freeze([
  "glbRegistration",
  "runtimePreviewBinding"
]);

export const treeEucalyptusGlbImportBridgeFoundationDefinition = deepFreeze({
  glbRegistration: {
  ...
  runtimePreviewBinding: {
    assetId: "TREE_EUCALYPTUS_001",
    renderPayload: {
      rendererAssetReference: "TREE_EUCALYPTUS_001",
      primaryGlb:
        "asset-factory-workspace/production/COASTAL_NATURE_FAMILY_001/export/TREE_EUCALYPTUS_001_LOD_CLOSE.glb",
...
export function createTreeEucalyptusGlbImportBridgeFoundation(
  rawDefinition = treeEucalyptusGlbImportBridgeFoundationDefinition
) {
  return normalizeBridgeFoundation(rawDefinition);
}

export function validateTreeEucalyptusGlbImportBridgeFoundation(
  rawDefinition = treeEucalyptusGlbImportBridgeFoundationDefinition,
  options = {}
) {
  try {
    ...
    const assetPackageDefinition =
      options.assetPackageDefinition ?? treeEucalyptusPrototypeAssetPackageDefinition;
    const localGeneratorDefinition =
      options.localGeneratorDefinition ?? treeEucalyptusLocalGeneratorDefinition;
    const existsSync =
      typeof options.existsSync === "function" ? options.existsSync : defaultExistsSync;

    const packageResult =
      validateTreeEucalyptusPrototypeAssetPackage(assetPackageDefinition);
    if (!packageResult.ok) {
      return freezeFailure(packageResult);
    }

    const generatorResult =
      validateTreeEucalyptusLocalGenerator(localGeneratorDefinition, {
    ...
    if (error?.name !== "TreeEucalyptusGlbImportBridgeFoundationValidationError") {
      throw error;
    }
...
function validateCompatibility(definition, assetPackage, localGenerator, existsSync) {
  ...
  if (registration.assetId !== assetPackage.assetId || registration.assetId !== previewBinding.assetId) {
    throw createValidationError(
      ...
      "Tree GLB import bridge asset identity must match the approved asset package and runtime preview binding."
    );
  }
  ...
  if (registration.glbPath !== registration.lodReferences.LOD_CLOSE) {
    throw createValidationError(
      ...
      "Tree GLB primary glbPath must match the close LOD reference."
    );
  }
  ...
  for (const [lodLabel, expectedPath] of Object.entries(expectedLodPaths)) {
    if (registration.lodReferences[lodLabel] !== expectedPath) {
      throw createValidationError(
        ...
        `Tree GLB registration ${lodLabel} must match the approved prototype asset package export output.`
      );
    }
  }

  if (
    path.basename(registration.manifestReference) !== "tree-eucalyptus-manifest.json" ||
    path.basename(registration.metadataReference) !== "tree-eucalyptus-metadata.json"
  ) {
    throw createValidationError(
      "metadata_linkage_mismatch",
      "Tree GLB import bridge must preserve the approved manifest and metadata filenames."
    );
  }
  ...
  if (
    ...
    throw createValidationError(
      ...
      "Tree runtime preview binding must point to the registered primary GLB and passive renderer profile."
    );
  }
}
...
function validateFileExistence(registration, existsSync) {
  ...
  if (missingPaths.length > 0) {
    throw createValidationError(
      "glb_missing",
      `Tree GLB import bridge requires existing files before registration: ${missingPaths.join(", ")}`
    );
  }
}

function normalizeBridgeFoundation(rawDefinition) {
  const definition = asPlainObject(rawDefinition, "treeEucalyptusGlbImportBridgeFoundation");
  for (const fieldName of treeEucalyptusGlbImportBridgeFoundationRequiredFields) {
    if (!(fieldName in definition)) {
      throw createValidationError(
        "missing_required_field",
        `Tree GLB import bridge is missing ${fieldName}.`
      );
    }
  }
...
function normalizeRuntimePreviewBinding(rawValue) {
  ...
  if (!supportedAppearanceProfiles.has(appearanceProfile)) {
    throw createValidationError(
      ...
      "Tree runtime preview binding appearanceProfile must be supported."
    );
  }
...
function createValidationError(code, message) {
  ...
  error.name = "TreeEucalyptusGlbImportBridgeFoundationValidationError";
  error.code = code;
  return error;
}
