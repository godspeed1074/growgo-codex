...
export const treeEucalyptusLocalGeneratorRequiredFields = Object.freeze([
  "assetId",
  "scriptLocation",
...
export const treeEucalyptusLocalGeneratorDefinition = deepFreeze({
  assetId: "TREE_EUCALYPTUS_001",
  scriptLocation:
    "asset-factory/local-blender-scripts/generate_tree_eucalyptus.py",
  expectedOutputLocation:
    "asset-factory-workspace/production/COASTAL_NATURE_FAMILY_001/export",
  executionInstructions: {
    blenderCommand:
      "blender --background --python asset-factory/local-blender-scripts/generate_tree_eucalyptus.py -- --output-dir asset-factory-workspace/production/COASTAL_NATURE_FAMILY_001/export",
    commandArguments: [
      "--background",
      "--python",
      "asset-factory/local-blender-scripts/generate_tree_eucalyptus.py",
...
export function createTreeEucalyptusLocalGenerator(
  rawDefinition = treeEucalyptusLocalGeneratorDefinition
) {
  return normalizeGeneratorDefinition(rawDefinition);
}

export function buildTreeEucalyptusLocalBlenderCommand(
  rawDefinition = treeEucalyptusLocalGeneratorDefinition
) {
...
export function validateTreeEucalyptusLocalGenerator(
  rawDefinition = treeEucalyptusLocalGeneratorDefinition,
  options = {}
) {
  try {
    ...
    if (error?.name !== "TreeEucalyptusLocalGeneratorValidationError") {
      throw error;
    }
...
function validateCompatibility(definition, assetPackage) {
  if (definition.assetId !== assetPackage.assetId) {
    throw createValidationError(
      "asset_id_mismatch",
      "Tree eucalyptus local generator assetId must match the prototype asset package."
    );
  }
  ...
  if (definition.expectedOutputLocation !== assetPackage.exportMetadata.assetPackageLocation) {
    throw createValidationError(
      ...
      "Tree eucalyptus local generator output location must match the prototype asset package export location."
    );
  }
  ...
  if (definition.scriptLocation !== expectedScriptPath) {
    throw createValidationError(
      ...
      "Tree eucalyptus local generator script location must match the approved Python script."
    );
  }
  ...
  if (!definition.executionInstructions.blenderCommand.startsWith("blender ")) {
    throw createValidationError(
      "command_mismatch",
      "Tree eucalyptus local generator command must start with the Blender executable."
    );
  }
  ...
  if (
    ...
    throw createValidationError(
      "command_mismatch",
      "Tree eucalyptus local generator command must include the script location and output directory."
    );
  }
  ...
  if (!definition.executionInstructions.localExecutionOnly) {
    throw createValidationError(
      ...
      "Tree eucalyptus local generator must remain local-execution-only."
    );
  }
}

function normalizeGeneratorDefinition(rawDefinition) {
  const definition = asPlainObject(rawDefinition, "tree eucalyptus local generator");
...
function assertRequiredFields(definition) {
  for (const fieldName of treeEucalyptusLocalGeneratorRequiredFields) {
    if (!Object.prototype.hasOwnProperty.call(definition, fieldName)) {
      throw createValidationError(
        "missing_required_field",
        `Tree eucalyptus local generator is missing required field ${fieldName}.`
      );
    }
  }
}
...
function createValidationError(code, message) {
  return Object.assign(new Error(message), {
    code,
    name: "TreeEucalyptusLocalGeneratorValidationError"
  });
}
