...
export function verifyTreeEucalyptusV003(options = {}) {
  ...
  return Object.freeze({
    schemaId: "TREE_EUCALYPTUS_V003_PRODUCTION_VERIFICATION_001",
    assetId: ASSET_ID,
    previousVersion: "v002",
...
export function writeTreeEucalyptusV003Verification(options = {}) {
  const cwd = options.cwd ?? process.cwd();
  const record = verifyTreeEucalyptusV003({ cwd });
  const outputDirectory = path.resolve(cwd, OUT);
  const gameplayReason = record.lods.GAMEPLAY.triangleCount > LIMITS.GAMEPLAY.preferredMax
    ? "Additional merged foliage-spray faces materially improve canopy cluster cohesion, drooping leaf silhouette, and crown readability in the gameplay view."
    : null;
  const validation = {
    ...record,
  ...
  fs.writeFileSync(path.join(outputDirectory, "tree-eucalyptus-v003-validation.json"), `${JSON.stringify(validation, null, 2)}\n`);
  fs.writeFileSync(path.join(outputDirectory, "tree-eucalyptus-v003-verification.json"), `${JSON.stringify({ schemaId: record.schemaId, assetId: ASSET_ID, checks: record.checks, technicalValidationPassed: record.technicalValidationPassed, deterministicFingerprint: hash(Buffer.from(JSON.stringify(record))) }, null, 2)}\n`);
  return validation;
}

if (import.meta.url === `file://${process.argv[1]}`) {
  console.log(JSON.stringify(writeTreeEucalyptusV003Verification(), null, 2));
}
