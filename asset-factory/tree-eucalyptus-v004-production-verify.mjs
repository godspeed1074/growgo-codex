...
export function verifyTreeEucalyptusV004(options = {}) {
  ...
  return Object.freeze({
    schemaId: "TREE_EUCALYPTUS_V004_PRODUCTION_VERIFICATION_001",
    assetId: ASSET_ID,
    previousVersion: "v003",
...
export function writeTreeEucalyptusV004Verification(options = {}) {
  const cwd = options.cwd ?? process.cwd();
  const record = verifyTreeEucalyptusV004({ cwd });
  const outputDirectory = path.resolve(cwd, OUT);
  const gameplayReason = record.lods.GAMEPLAY.triangleCount > LIMITS.GAMEPLAY.preferredMax
    ? "Additional manifold papercut plate faces materially preserve front, side, and top canopy mass silhouettes."
    : null;
  const validation = {
    ...record,
  ...
  fs.writeFileSync(path.join(outputDirectory, "tree-eucalyptus-v004-validation.json"), `${JSON.stringify(validation, null, 2)}\n`);
  fs.writeFileSync(path.join(outputDirectory, "tree-eucalyptus-v004-verification.json"), `${JSON.stringify({ schemaId: record.schemaId, assetId: ASSET_ID, checks: record.checks, technicalValidationPassed: record.technicalValidationPassed, deterministicFingerprint: hash(Buffer.from(JSON.stringify(record))) }, null, 2)}\n`);
  return validation;
}

if (import.meta.url === `file://${process.argv[1]}`) {
  console.log(JSON.stringify(writeTreeEucalyptusV004Verification(), null, 2));
}
