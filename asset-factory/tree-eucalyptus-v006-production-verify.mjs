...
export function verifyTreeEucalyptusV006(options = {}) {
  ...
  return Object.freeze({
    schemaId: "TREE_EUCALYPTUS_V006_PRODUCTION_VERIFICATION_001",
    assetId: ASSET_ID,
    previousVersion: "v005",
...
export function writeTreeEucalyptusV006Verification(options = {}) {
  const cwd = options.cwd ?? process.cwd();
  const record = verifyTreeEucalyptusV006({ cwd });
  const outputDirectory = path.resolve(cwd, OUT);
  const gameplayReason = record.lods.GAMEPLAY.triangleCount > LIMITS.GAMEPLAY.preferredMax
    ? "Additional authored card planes materially preserve side/top canopy depth and the approved concept silhouette."
    : null;
  const validation = {
    ...record,
  ...
  fs.writeFileSync(path.join(outputDirectory, "tree-eucalyptus-v006-validation.json"), `${JSON.stringify(validation, null, 2)}\n`);
  fs.writeFileSync(path.join(outputDirectory, "tree-eucalyptus-v006-verification.json"), `${JSON.stringify({ schemaId: record.schemaId, assetId: ASSET_ID, checks: record.checks, technicalValidationPassed: record.technicalValidationPassed, deterministicFingerprint: hash(Buffer.from(JSON.stringify(record))) }, null, 2)}\n`);
  return validation;
}

if (import.meta.url === `file://${process.argv[1]}`) {
  console.log(JSON.stringify(writeTreeEucalyptusV006Verification(), null, 2));
}
