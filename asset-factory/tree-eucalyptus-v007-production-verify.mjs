...
export function verifyTreeEucalyptusV007(options = {}) {
  ...
  check("gameplay_in_v007_density_target", lods.GAMEPLAY.triangleCount >= 2500 && lods.GAMEPLAY.triangleCount <= 4500, `${lods.GAMEPLAY.triangleCount} triangles`);
  ...
  return Object.freeze({
    schemaId: "TREE_EUCALYPTUS_V007_PRODUCTION_VERIFICATION_001",
    assetId: ASSET_ID,
    previousVersion: "v006",
...
export function writeTreeEucalyptusV007Verification(options = {}) {
  const cwd = options.cwd ?? process.cwd();
  const record = verifyTreeEucalyptusV007({ cwd });
  const outputDirectory = path.resolve(cwd, OUT);
  const gameplayReason = record.lods.GAMEPLAY.triangleCount > LIMITS.GAMEPLAY.preferredMax
    ? "Additional authored card planes materially preserve the requested fuller front/middle/back canopy layering."
    : null;
  const validation = {
    ...record,
  ...
  fs.writeFileSync(path.join(outputDirectory, "tree-eucalyptus-v007-validation.json"), `${JSON.stringify(validation, null, 2)}\n`);
  fs.writeFileSync(path.join(outputDirectory, "tree-eucalyptus-v007-verification.json"), `${JSON.stringify({ schemaId: record.schemaId, assetId: ASSET_ID, checks: record.checks, technicalValidationPassed: record.technicalValidationPassed, deterministicFingerprint: hash(Buffer.from(JSON.stringify(record))) }, null, 2)}\n`);
  return validation;
}

if (import.meta.url === `file://${process.argv[1]}`) {
  console.log(JSON.stringify(writeTreeEucalyptusV007Verification(), null, 2));
}
