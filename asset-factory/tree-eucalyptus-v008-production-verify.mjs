...
export function verifyTreeEucalyptusV008(options = {}) {
  ...
  check("gameplay_in_v008_reference_match_band", lods.GAMEPLAY.triangleCount >= 3500 && lods.GAMEPLAY.triangleCount <= 6500, `${lods.GAMEPLAY.triangleCount} triangles`);
  ...
  return Object.freeze({
    schemaId: "TREE_EUCALYPTUS_V008_PRODUCTION_VERIFICATION_001",
    assetId: ASSET_ID,
...
export function writeTreeEucalyptusV008Verification(options = {}) {
  const cwd = options.cwd ?? process.cwd();
  const record = verifyTreeEucalyptusV008({ cwd });
  const outputDirectory = path.resolve(cwd, OUT);
  const gameplayReason = record.lods.GAMEPLAY.triangleCount > LIMITS.GAMEPLAY.preferredMax
    ? "Additional authored umbrella layers, sculpted fork swells, and rounded root toes materially improve reference-shape similarity."
    : null;
  const validation = {
    ...record,
  ...
  fs.writeFileSync(path.join(outputDirectory, "tree-eucalyptus-v008-validation.json"), `${JSON.stringify(validation, null, 2)}\n`);
  fs.writeFileSync(path.join(outputDirectory, "tree-eucalyptus-v008-verification.json"), `${JSON.stringify({ schemaId: record.schemaId, assetId: ASSET_ID, checks: record.checks, technicalValidationPassed: record.technicalValidationPassed, deterministicFingerprint: hash(Buffer.from(JSON.stringify(record))) }, null, 2)}\n`);
  return validation;
}

if (import.meta.url === `file://${process.argv[1]}`) {
  console.log(JSON.stringify(writeTreeEucalyptusV008Verification(), null, 2));
}
