...
export function verifyTreeEucalyptusV010(options = {}) {
  ...
  check("crown_depth_in_target_band", lods.GAMEPLAY.bounds.dimensions[2] >= 6.0 && lods.GAMEPLAY.bounds.dimensions[2] <= 8.2, `depth=${lods.GAMEPLAY.bounds.dimensions[2]}`);
  ...
  check("gameplay_in_v010_refinement_band", lods.GAMEPLAY.triangleCount >= 5500 && lods.GAMEPLAY.triangleCount <= 8000, `${lods.GAMEPLAY.triangleCount} triangles`);
  ...
  return Object.freeze({
    schemaId: "TREE_EUCALYPTUS_V010_PRODUCTION_VERIFICATION_001",
    assetId: ASSET_ID,
...
export function writeTreeEucalyptusV010Verification(options = {}) {
  const cwd = options.cwd ?? process.cwd();
  const record = verifyTreeEucalyptusV010({ cwd });
  const outputDirectory = path.resolve(cwd, OUT);
  ...
  fs.writeFileSync(path.join(outputDirectory, "tree-eucalyptus-v010-validation.json"), `${JSON.stringify(validation, null, 2)}\n`);
  fs.writeFileSync(path.join(outputDirectory, "tree-eucalyptus-v010-verification.json"), `${JSON.stringify({ schemaId: record.schemaId, assetId: ASSET_ID, checks: record.checks, technicalValidationPassed: record.technicalValidationPassed, deterministicFingerprint: hash(Buffer.from(JSON.stringify(record))) }, null, 2)}\n`);
  return validation;
}

if (import.meta.url === `file://${process.argv[1]}`) {
  console.log(JSON.stringify(writeTreeEucalyptusV010Verification(), null, 2));
}
