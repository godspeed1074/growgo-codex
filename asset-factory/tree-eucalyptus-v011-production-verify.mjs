...
const VERSION = "v011";
const OUT = "/Users/michaelpeterson/Developer/growgo-codex/asset-factory-workspace/production/COASTAL_NATURE_FAMILY_001/export";
const TEXTURE = "/Users/michaelpeterson/Developer/growgo-codex/asset-factory-workspace/production/COASTAL_NATURE_FAMILY_001/textures/TREE_EUCALYPTUS_001_v006_FOLIAGE_ATLAS_512.png";
...
export function verifyTreeEucalyptusV011(options = {}) {
  ...
  const previousFiles = [
    ...
    `${ASSET_ID}_v009.blend`, `${ASSET_ID}_v009_LOD_CLOSE.glb`,
    `${ASSET_ID}_v009_LOD_GAMEPLAY.glb`, `${ASSET_ID}_v009_LOD_MAP.glb`
  ].map((filename) => {
  ...
  check("authored_height_coherent", lods.CLOSE.bounds.max[1] >= 12.8 && lods.CLOSE.bounds.max[1] <= 14.2, `maxY=${lods.CLOSE.bounds.max[1]}`);
  check("crown_width_in_target_band", lods.GAMEPLAY.bounds.dimensions[0] >= 7.5 && lods.GAMEPLAY.bounds.dimensions[0] <= 10.5, `width=${lods.GAMEPLAY.bounds.dimensions[0]}`);
  check("crown_depth_in_target_band", lods.GAMEPLAY.bounds.dimensions[2] >= 5.0 && lods.GAMEPLAY.bounds.dimensions[2] <= 8.2, `depth=${lods.GAMEPLAY.bounds.dimensions[2]}`);
  ...
  check("gameplay_in_v011_refinement_band", lods.GAMEPLAY.triangleCount >= 5500 && lods.GAMEPLAY.triangleCount <= 8000, `${lods.GAMEPLAY.triangleCount} triangles`);
  ...
  return Object.freeze({
    schemaId: "TREE_EUCALYPTUS_V011_PRODUCTION_VERIFICATION_001",
    assetId: ASSET_ID,
...
export function writeTreeEucalyptusV011Verification(options = {}) {
  const cwd = options.cwd ?? process.cwd();
  const record = verifyTreeEucalyptusV011({ cwd });
  const outputDirectory = path.resolve(cwd, OUT);
  ...
  fs.writeFileSync(path.join(outputDirectory, "tree-eucalyptus-v011-validation.json"), `${JSON.stringify(validation, null, 2)}\n`);
  fs.writeFileSync(path.join(outputDirectory, "tree-eucalyptus-v011-verification.json"), `${JSON.stringify({ schemaId: record.schemaId, assetId: ASSET_ID, checks: record.checks, technicalValidationPassed: record.technicalValidationPassed, deterministicFingerprint: hash(Buffer.from(JSON.stringify(record))) }, null, 2)}\n`);
  return validation;
}

if (import.meta.url === `file://${process.argv[1]}`) {
  console.log(JSON.stringify(writeTreeEucalyptusV011Verification(), null, 2));
}
