...
function createAdapterLayer() {
  ...
  return adapterModule.createAtlasRendererAdapterLayer(handoffLayer);
}

test("activation is blocked without renderer authorization", () => {
  const layer = activationModule.createAtlasControlledRendererActivationLayer(
    createAdapterLayer(),
    { lifecycleOwnerId: "ATLAS_PREP_TEAM" }
  );

  assert.equal(layer.activationReadiness.status, "BLOCKED");
  assert.equal(layer.activationGates.rendererAuthorization.granted, false);
  assert.equal(layer.activationPolicy.automaticActivationAllowed, false);
});

test("activation is ready only when all gates pass", () => {
  const layer = activationModule.createAtlasControlledRendererActivationLayer(
    createAdapterLayer(),
    {
      lifecycleOwnerId: "ATLAS_PREP_TEAM",
  ...
  assert.equal(layer.activationReadiness.status, "READY");
  assert.equal(layer.activationReadiness.readyForControlledTest, true);
  assert.equal(layer.activationGates.sceneValidationPassed, true);
  assert.equal(layer.activationGates.assetValidationPassed, true);
  assert.equal(layer.activationGates.commandValidationPassed, true);
  assert.equal(layer.activationGates.lifecycleOwnershipConfirmed, true);
});

test("cleanup requirements exist and controlled test scene is single-object and reversible", () => {
  const layer = activationModule.createAtlasControlledRendererActivationLayer(
    createAdapterLayer(),
    {
      lifecycleOwnerId: "ATLAS_PREP_TEAM",
  ...
  assert.equal(layer.cleanupRequirements.cleanupRequired, true);
  assert.equal(layer.cleanupRequirements.reversibleExecutionRequired, true);
  assert.equal(layer.controlledRendererTestScene.objectCount, 1);
  assert.equal(layer.controlledRendererTestScene.nonProductionData, true);
  assert.equal(layer.controlledRendererTestScene.reversibleExecution, true);
});

test("same inputs produce deterministic controlled activation preparation state", () => {
  const first = activationModule.createAtlasControlledRendererActivationLayer(
    createAdapterLayer(),
    {
      lifecycleOwnerId: "ATLAS_PREP_TEAM",
      rendererAuthorization: "AUTHORIZED_CONTROLLED_RENDER_TEST"
  ...
  const second = activationModule.createAtlasControlledRendererActivationLayer(
    createAdapterLayer(),
    {
      lifecycleOwnerId: "ATLAS_PREP_TEAM",
      rendererAuthorization: "AUTHORIZED_CONTROLLED_RENDER_TEST"
  ...
  assert.equal(first.validation.deterministicPreparationState, true);
});
