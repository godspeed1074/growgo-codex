...
function createPreviewLayer() {
  return previewModule.createAtlasEnvironmentPreviewLayer(
    createDirectWorldObjects(),
...
test("suburban street scene composes houses gardens and road context", () => {
  const layer = sceneModule.createAtlasStreetSceneCompositionLayer(
    createDirectWorldObjects(),
    createDirectRelationships(),
    createPreviewLayer()
  );
  const scene = layer.streetScenes.entries.find(
    (entry) => entry.sceneType === "SUBURBAN_STREET_SCENE"
  );

  assert.ok(scene);
  assert.ok(scene.objectList.some((entry) => entry.objectType === "HOUSE"));
  assert.ok(scene.objectList.some((entry) => entry.objectType === "TRANSPORT_ROUTE"));
  assert.equal(scene.sceneMetadata.includesGardens, true);
});

test("town main street scene composes businesses with street-serving transport", () => {
  const layer = sceneModule.createAtlasStreetSceneCompositionLayer(
    createDirectWorldObjects(),
    createDirectRelationships(),
    createPreviewLayer()
  ...
  assert.ok(scene);
  assert.ok(scene.objectList.some((entry) => entry.objectType === "BAKERY"));
  assert.ok(scene.objectList.some((entry) => entry.objectType === "CAFE"));
  assert.ok(scene.sceneMetadata.relationshipTypes.includes("served_by_road"));
});

test("coastal street scene composes waterfront objects without changing geography", () => {
  const layer = sceneModule.createAtlasStreetSceneCompositionLayer(
    createDirectWorldObjects(),
    createDirectRelationships(),
    createPreviewLayer()
  ...
  assert.ok(scene);
  assert.ok(scene.objectList.some((entry) => entry.objectType === "BEACH"));
  assert.ok(scene.objectList.some((entry) => entry.objectType === "LIGHTHOUSE"));
  assert.equal(scene.sceneMetadata.waterfrontContext, true);
});

test("street scene composition validation passes contract checks", () => {
  const layer = sceneModule.createAtlasStreetSceneCompositionLayer(
    createDirectWorldObjects(),
    createDirectRelationships(),
    createPreviewLayer()
  );
  const validation = sceneModule.validateAtlasStreetSceneCompositionLayer(layer);

  assert.equal(validation.ok, true);
  assert.equal(validation.atlasStreetSceneCompositionLayer.validation.objectsBelongToScene, true);
  assert.equal(validation.atlasStreetSceneCompositionLayer.validation.geometryPreserved, true);
});

test("same inputs produce deterministic same street scene composition", () => {
  const first = sceneModule.createAtlasStreetSceneCompositionLayer(
    createDirectWorldObjects(),
    createDirectRelationships(),
    createPreviewLayer()
  );
  const second = sceneModule.createAtlasStreetSceneCompositionLayer(
    createDirectWorldObjects(),
    createDirectRelationships(),
    createPreviewLayer()
  );
