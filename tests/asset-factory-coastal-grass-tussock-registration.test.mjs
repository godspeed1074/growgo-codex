...
test("grass v001 visual approval record exists and is approved", () => {
  const approval = readJson("coastal-grass-tussock-visual-approval.json");

  assert.equal(approval.assetId, "COASTAL_GRASS_TUSSOCK_001");
...
test("grass registration record links v001 evidence and preserves safe non-published state", () => {
  const registration = readJson("coastal-grass-tussock-registration.json");

  assert.equal(registration.assetId, "COASTAL_GRASS_TUSSOCK_001");
...
test("grass development catalog stays development-only and blocks runtime or publish activation", () => {
...
test("grass version record tracks v001 lifecycle state without publish or runtime activation", () => {
  const versionRecord = readJson("coastal-grass-tussock-v001-version-record.json");

  assert.equal(versionRecord.assetId, "COASTAL_GRASS_TUSSOCK_001");
