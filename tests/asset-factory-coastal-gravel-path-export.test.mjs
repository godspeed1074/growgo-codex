...
test("coastal gravel path export validation records exported glbs and lod ordering", () => {
  const exportValidation = JSON.parse(fs.readFileSync(exportValidationPath, "utf8"));

  assert.equal(exportValidation.assetId, "COASTAL_GRAVEL_PATH_001");
  ...
  assert.deepEqual(
    ...
    [
      "COASTAL_GRAVEL_PATH_001_LOD_CLOSE.glb",
      "COASTAL_GRAVEL_PATH_001_LOD_GAMEPLAY.glb",
      "COASTAL_GRAVEL_PATH_001_LOD_MAP.glb"
    ]
  );
...
test("coastal gravel path export manifest matches recorded validation metrics", () => {
  const exportValidation = JSON.parse(fs.readFileSync(exportValidationPath, "utf8"));
  const exportManifest = JSON.parse(
    fs.readFileSync(path.join(exportRoot, "coastal-gravel-path-export-manifest.json"), "utf8")
  );

  assert.equal(exportManifest.version, "v001");
  assert.equal(exportManifest.outputs.close.triangleCount, 137);
  assert.equal(exportManifest.outputs.gameplay.triangleCount, 113);
  assert.equal(exportManifest.outputs.map.triangleCount, 89);
  assert.equal(exportManifest.outputs.close.meshCount, 11);
...
test("coastal gravel path glbs preserve per-lod anchors and have no external dependencies", () => {
  const expectations = [
    [
      "COASTAL_GRAVEL_PATH_001_LOD_CLOSE.glb",
      "COASTAL_GRAVEL_PATH_001_LOD_CLOSE_IDENTITY_ANCHOR",
      11
    ],
    [
      "COASTAL_GRAVEL_PATH_001_LOD_GAMEPLAY.glb",
      "COASTAL_GRAVEL_PATH_001_LOD_GAMEPLAY_IDENTITY_ANCHOR",
      9
    ],
    [
      "COASTAL_GRAVEL_PATH_001_LOD_MAP.glb",
      "COASTAL_GRAVEL_PATH_001_LOD_MAP_IDENTITY_ANCHOR",
      7
    ]
  ];
  ...
  for (const [filename, anchorName, primitiveCount] of expectations) {
    ...
    assert.match(blob, /COASTAL_GRAVEL_PATH_001/);
    assert.match(blob, /COASTAL_GRAVEL_PATH_RECIPE_001/);
...
test("coastal gravel path export report documents verified completion and registration readiness", () => {
  const report = fs.readFileSync(reportPath, "utf8");

  assert.match(report, /Verified complete/i);
  assert.match(report, /source blend hash remained unchanged/i);
  assert.match(report, /COASTAL_GRAVEL_PATH_001_LOD_CLOSE\.glb/);
  assert.match(report, /COASTAL_GRAVEL_PATH_001_LOD_GAMEPLAY\.glb/);
  assert.match(report, /COASTAL_GRAVEL_PATH_001_LOD_MAP\.glb/);
