...
const intakePath = path.join(
  productionRoot,
  "specification/coastal-gravel-path-001-intake.json"
);
const validationPath = path.join(
  productionRoot,
  "validation/coastal-gravel-path-001-intake-validation.json"
);
const reportPath = path.join(
  productionRoot,
  "reports/coastal-gravel-path-001-intake-report.md"
);

test("coastal gravel path intake defines identity, recipe, family, and LOD expectations", () => {
  const intake = JSON.parse(fs.readFileSync(intakePath, "utf8"));

  assert.equal(intake.assetIdentity.assetId, "COASTAL_GRAVEL_PATH_001");
  assert.equal(intake.assetIdentity.familyId, "COASTAL_PATHWAY_FAMILY_001");
  assert.equal(intake.recipeIdentity.recipeId, "COASTAL_GRAVEL_PATH_RECIPE_001");
...
test("coastal gravel path intake defines modular coastal pathway budgets and dependencies", () => {
  const intake = JSON.parse(fs.readFileSync(intakePath, "utf8"));
...
test("coastal gravel path intake defines source/export separation and pre-export gates and remains pre-authoring", () => {
  const intake = JSON.parse(fs.readFileSync(intakePath, "utf8"));
  ...
  assert.equal(intake.authoringReadiness.nextPhase, "198.2_authoring_setup");
});

test("coastal gravel path validation confirms intake-only state", () => {
  const validation = JSON.parse(fs.readFileSync(validationPath, "utf8"));

  assert.equal(validation.assetId, "COASTAL_GRAVEL_PATH_001");
...
test("coastal gravel path report documents intake completion and readiness for authoring setup", () => {
  const report = fs.readFileSync(reportPath, "utf8");

  assert.match(report, /Intake complete, authoring not started/);
  assert.match(report, /visual approval required before export/i);
  assert.match(report, /source path verification required before export/i);
  assert.match(report, /No Blender file, geometry, GLB, registration, or promotion artifact has been created/i);
  assert.match(report, /ready for Phase 198\.2 authoring setup/i);
});
