...
test("coastal rock cluster generator and resume scripts are valid Python syntax", () => {
...
test("generator preserves identity and creates every required root and anchor", () => {
  ...
  assert.match(script, /ASSET_ID = "COASTAL_ROCK_CLUSTER_001"/);
  assert.match(script, /SOURCE_RECIPE_ID = "COASTAL_ROCK_CLUSTER_RECIPE_001"/);
  assert.match(script, /ASSET_VERSION = "v001"/);
  assert.match(script, /PALETTE_ID = "AU_COASTAL_ROCK_001"/);
  ...
  assert.match(script, /COASTAL_ROCK_CLUSTER_001_LOD_CLOSE_IDENTITY_ANCHOR/);
  assert.match(script, /COASTAL_ROCK_CLUSTER_001_LOD_GAMEPLAY_IDENTITY_ANCHOR/);
  assert.match(script, /COASTAL_ROCK_CLUSTER_001_LOD_MAP_IDENTITY_ANCHOR/);
  assert.match(script, /create_identity_anchor\(/);
  assert.match(script, /required_helpers=\("asset_identity_anchor_v2",\)/);
});

test("generator defines deterministic mobile-first papercut rock-cluster geometry", () => {
  const script = fs.readFileSync(generatorPath, "utf8");

  assert.match(script, /CLOSE_CORE_SPECS = \(/);
  assert.match(script, /GAMEPLAY_CORE_SPECS = \(/);
  assert.match(script, /MAP_CORE_SPECS = \(/);
  assert.match(script, /\(-0\.46, 0\.12, 0\.22\)/);
  assert.match(script, /\(0\.22, 0\.10, 0\.34\)/);
  assert.match(script, /\(0\.22, -0\.02, 0\.22\)/);
  ...
  assert.match(script, /natural silhouettes that repeat well/);
...
test("generator prepares three LOD roots but never exports GLBs", () => {
  ...
  assert.match(script, /coastal-rock-cluster-authoring-manifest\.json/);
...
test("resume workflow is deterministic identity-aware and universal-exporter based", () => {
  ...
  assert.match(script, /f"\{ASSET_ID\}_LOD_GAMEPLAY\.glb"/);
  assert.match(script, /f"\{ASSET_ID\}_LOD_MAP\.glb"/);
  assert.match(script, /coastal-rock-cluster-export-manifest\.json/);
...
test("manual authoring setup record preserves identity, folder contract, and setup-only safety", () => {
  const setup = JSON.parse(
    fs.readFileSync(
      path.join(
        productionRoot,
        "validation/coastal-rock-cluster-manual-authoring-setup.json"
      ),
      "utf8"
    )
  );

  assert.equal(setup.assetId, "COASTAL_ROCK_CLUSTER_001");
  assert.equal(setup.version, "v001");
  assert.equal(setup.expectedBlend, "COASTAL_ROCK_CLUSTER_001_v001.blend");
  assert.equal(
    setup.expectedBlendLocation,
    "asset-factory-workspace/production/COASTAL_NATURE_FAMILY_001/source/COASTAL_ROCK_CLUSTER_001_v001.blend"
  );
  assert.deepEqual(setup.expectedLodRoots, [
    "COASTAL_ROCK_CLUSTER_001_LOD_CLOSE_ROOT",
    "COASTAL_ROCK_CLUSTER_001_LOD_GAMEPLAY_ROOT",
    "COASTAL_ROCK_CLUSTER_001_LOD_MAP_ROOT"
  ]);
  assert.equal(
    setup.folderContract.blendFilesAllowedInExport,
    false
  );
...
test("coastal rock cluster source/export contract keeps source canonical and export free of blend files", () => {
  assert.equal(
    fs.existsSync(
      path.join(productionRoot, "source/COASTAL_ROCK_CLUSTER_001_v001.blend")
    ),
    true
  );
  assert.equal(
