...
const protectedFiles = [
  ...
  path.join(exportRoot, "COASTAL_ROCK_CLUSTER_001_LOD_CLOSE.glb"),
  path.join(exportRoot, "COASTAL_ROCK_CLUSTER_001_LOD_GAMEPLAY.glb"),
  path.join(exportRoot, "COASTAL_ROCK_CLUSTER_001_LOD_MAP.glb")
];
...
test("rock cluster v001 promotion record sets active development revision", () => {
  const promotion = readJson("coastal-rock-cluster-v001-promotion.json");

  assert.equal(promotion.assetId, "COASTAL_ROCK_CLUSTER_001");
...
test("rock cluster development catalog moves from current candidate to active development revision", () => {
...
test("rock cluster version state marks v001 active and promotion performed", () => {
  ...
  assert.equal(versionRecord.v001.promotionRecord, "coastal-rock-cluster-v001-promotion.json");
...
test("promotion state does not modify protected rock-cluster source or exported glbs", () => {
  const before = Object.fromEntries(protectedFiles.map((filepath) => [filepath, hash(filepath)]));
  readJson("coastal-rock-cluster-v001-promotion.json");
