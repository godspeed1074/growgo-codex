...
const protectedFiles = [
  ...
  path.join(exportRoot, "COASTAL_WATER_EDGE_001_LOD_CLOSE.glb"),
  path.join(exportRoot, "COASTAL_WATER_EDGE_001_LOD_GAMEPLAY.glb"),
  path.join(exportRoot, "COASTAL_WATER_EDGE_001_LOD_MAP.glb")
];
...
const promotionModule = await import(
  path.resolve(
    ...
    "..",
    "asset-factory",
    "coastal-water-edge-promotion.mjs"
  )
);
...
test("water edge promotion builder derives active development revision state", () => {
  const promotion = promotionModule.buildCoastalWaterEdgeV001Promotion({
    cwd: repoRoot
  });

  assert.equal(promotion.registration.assetId, "COASTAL_WATER_EDGE_001");
...
test("water edge promotion record sets active development revision", () => {
  const promotion = readJson("coastal-water-edge-v001-promotion.json");

  assert.equal(promotion.assetId, "COASTAL_WATER_EDGE_001");
...
test("water edge development catalog moves from current candidate to active development revision", () => {
...
test("water edge version state marks v001 active and promotion performed", () => {
  ...
  assert.equal(
    ...
    "coastal-water-edge-v001-promotion.json"
  );
...
test("promotion state does not modify protected water-edge source or exported glbs", () => {
  const before = Object.fromEntries(protectedFiles.map((filepath) => [filepath, hash(filepath)]));
  readJson("coastal-water-edge-v001-promotion.json");
