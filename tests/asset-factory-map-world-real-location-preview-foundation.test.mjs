...
function createSyntheticGlb({
  materialNames = ["LocationPreviewMaterialA", "LocationPreviewMaterialB"]
} = {}) {
...
test("map world real location preview foundation validates a coordinate-driven preview package", async () => {
  const foundation = await moduleUnderTest.createMapWorldRealLocationPreviewFoundation(
    moduleUnderTest.mapWorldRealLocationPreviewFoundationDefinition,
    buildLoaderOptions()
  );
  const result = moduleUnderTest.validateMapWorldRealLocationPreviewFoundation(foundation);

  assert.equal(result.ok, true);
  assert.equal(
    foundation.coordinate.latitude,
    foundation.mapWorldLiveMapFoundation.centerCoordinate.latitude
  );
  assert.equal(
    foundation.resolvedWorld.worldId,
    foundation.mapWorldLiveMapFoundation.activeWorldId
  );
  assert.equal(
    foundation.previewScene.sceneId,
    foundation.mapWorldLiveMapFoundation.mapWorldVisualLayerAttachment.sceneId
  );
});

test("same map position produces deterministic real-location preview output", async () => {
  const first = await moduleUnderTest.createMapWorldRealLocationPreviewFoundation(
    moduleUnderTest.mapWorldRealLocationPreviewFoundationDefinition,
    buildLoaderOptions()
  );
  const second = await moduleUnderTest.createMapWorldRealLocationPreviewFoundation(
    moduleUnderTest.mapWorldRealLocationPreviewFoundationDefinition,
    buildLoaderOptions()
  );

  assert.equal(first.locationPreviewId, second.locationPreviewId);
  assert.deepEqual(first.coordinate, second.coordinate);
  assert.equal(first.resolvedWorld.worldId, second.resolvedWorld.worldId);
});

test("real map positions now create world previews through the preview session", async () => {
  const session = moduleUnderTest.createMapWorldRealLocationPreviewSession({
  ...
  assert.match(selected.message, /Selected current map position for preview/i);
  ...
  const loadedPreview = session.loadGeneratedWorldPreview({
    document: createMockDocument(),
    previewMountOptions: buildPreviewMountOptions()
  });
  assert.equal(loadedPreview.ok, true);
  assert.ok(
    loadedPreview.atlasPreviewAttachment.atlasBrowserDemoHarness.canvas._context.commands.some(
      (command) =>
