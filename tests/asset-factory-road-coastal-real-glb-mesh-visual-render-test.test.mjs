...
const moduleUnderTest = await import(
  path.resolve(
    ...
    "..",
    "asset-factory",
    "road-coastal-real-glb-mesh-visual-render-test.mjs"
  )
);
...
async function buildRuntimeDefinition() {
  const definition = runtimeBindingModule.roadCoastalRuntimePreviewBindingDefinition;
  const existingPaths = [
  ...
  const result = await runtimeBindingModule.loadRoadCoastalRuntimePreviewBinding(
    definition,
    {
...
test("road coastal mesh visual render test reuses the existing preview renderer input shape", async () => {
  const runtimeDefinition = await buildRuntimeDefinition();
  const definition =
    moduleUnderTest.createRoadCoastalRealGlbMeshVisualRenderTest(runtimeDefinition);
  const result =
    moduleUnderTest.validateRoadCoastalRealGlbMeshVisualRenderTest(definition);

  assert.equal(result.ok, true);
  assert.equal(definition.assetId, "ROAD_COASTAL_001");
...
test("road coastal mesh visual render test rejects missing material mapping safely", async () => {
  const runtimeDefinition = await buildRuntimeDefinition();
  const definition =
    moduleUnderTest.createRoadCoastalRealGlbMeshVisualRenderTest(runtimeDefinition);
  ...
  const result =
    moduleUnderTest.validateRoadCoastalRealGlbMeshVisualRenderTest(invalidDefinition);
