...
test("shrub v002 visual approval record exists and is approved", () => {
  const approval = readJson("shrub-coastal-low-v002-visual-approval.json");

  assert.equal(approval.assetId, "SHRUB_COASTAL_LOW_001");
...
test("shrub registration record links v002 evidence and preserves v001 as historical protected", () => {
  const registration = readJson("shrub-coastal-low-registration.json");

  assert.equal(registration.assetId, "SHRUB_COASTAL_LOW_001");
...
test("shrub development catalog stays development-only and blocks runtime or publish activation", () => {
...
test("shrub version record keeps v001 historical approved and tracks current v002 state", () => {
  const versionRecord = readJson("shrub-coastal-low-v002-version-record.json");

  assert.equal(versionRecord.assetId, "SHRUB_COASTAL_LOW_001");
