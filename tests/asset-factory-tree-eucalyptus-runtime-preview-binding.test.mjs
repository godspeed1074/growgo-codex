...
test("tree eucalyptus runtime preview binding loads the gameplay GLB into the existing real GLB pipeline shape", async () => {
  const definition = moduleUnderTest.treeEucalyptusRuntimePreviewBindingDefinition;
  const existingPaths = [
  ...
  const result = await moduleUnderTest.loadTreeEucalyptusRuntimePreviewBinding(
    definition,
    {
...
test("tree eucalyptus runtime preview binding preserves fallback when the gameplay GLB is unavailable", async () => {
  const result = await moduleUnderTest.loadTreeEucalyptusRuntimePreviewBinding(
    moduleUnderTest.treeEucalyptusRuntimePreviewBindingDefinition,
    {
      existsSync: () => false,
      loadArrayBuffer() {
        return Promise.resolve(createSyntheticGlb());
