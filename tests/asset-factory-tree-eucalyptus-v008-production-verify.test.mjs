...
import { verifyTreeEucalyptusV008 } from "../asset-factory/tree-eucalyptus-v008-production-verify.mjs";

test("TREE_EUCALYPTUS_001 v008 is a protected, grounded, mobile-ready review candidate", () => {
  const result = verifyTreeEucalyptusV008();
  assert.equal(result.assetId, "TREE_EUCALYPTUS_001");
  assert.equal(result.previousVersion, "v007");
  assert.equal(result.targetVersion, "v008");
  ...
  assert.ok(result.lods.GAMEPLAY.triangleCount >= 3500);
